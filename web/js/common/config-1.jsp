...
<%
    ...
    try{
    	shts = dao.multiRowSelect("select modid, modname, modtag from cpcmod where modtag>0 and webrefaddr='crmman.sht'");

    	menushts = dao.multiRowSelect("select menuid modid, menuname modname, menutag modtag from cpcmenu where menutag>0 and webrefaddr='crmman.sht'");
...
function config($stateProvider, $$qProvider, $urlRouterProvider, $ocLazyLoadProvider) {
	...
	$stateProvider
			...
			resolve: {
				load: function($templateCache, $ocLazyLoad, $q, $http) {
					...
					return $ocLazyLoad.load(
						...
						{
							name: 'inspinia',
							files: ['js/controllers/baseman/company_files.js']
						}
		...
		//预算期间
		.state("finman.fin_bud_period_header", {
		...
		//预算类别
		.state("finman.fin_bud_type_header", {
	...
	//费用项目设置
		...
		//预算调整
		.state("finman.fin_bud_adjust_header",{
		...
		//预算结转
		.state("finman.fin_bud_carryover", {
		...
		//费用申请
		.state("finman.fin_fee_apply_header", {
		...
		//预算审核
		.state("finman.fin_bud_audit_head", {
		...
		//费用报销查询
		.state("finman.fin_fee_bx_header_search", {
		...
		//费用报销
		.state("finman.fin_fee_bx_header", {
		...
		//预算编制—变动
		.state("finman.fin_bud_make_change", {
		...
		//费用申请结案
		.state("finman.fin_fee_apply_header_over", {
	...
	//预算执行进度统计页面
	     .state("finman.fin_bud_execution_progress", {
	...
	//销售管理页面
		...
		.state("saleman.sale_yeartask_head", {
	...
	//		Hashtable ht = (Hashtable)shts.elementAt(i);
	//		out.println(".state('crmman.sht_"+(String)ht.get("modid")+"', {");
	//		out.println("    url: '/sht_"+(String)ht.get("modid")+"',");
	//		out.println("    templateUrl: 'views/baseman/sht_view.jsp?shtid="+(String)ht.get("modtag")+"',");
	//		out.println("    data: {");
	//		out.println("        pageTitle: '"+(String)ht.get("modname")+"'");
	//		out.println("    },");
	//		out.println("    resolve: {");
	//		out.println("        load: function($templateCache, $ocLazyLoad, $q, $http) {");
	//		out.println("            lazyDeferred = $q.defer();");
	//		out.println("            return $ocLazyLoad.load({");
	//		out.println("                name: 'inspinia',");
	//		out.println("                files: ['js/controllers/baseman/base_shtcontroner.jsp?shtid="+(String)ht.get("modtag")+"']");
	//		out.println("            });");
	//		out.println("        }");
	//		out.println("    }");
	//		out.println("})");
	...
	//if (menushts!=null && menushts.size()>0){
	//	for(int i=0; i<menushts.size(); i++){
	//		Hashtable ht = (Hashtable)menushts.elementAt(i);
	//		out.println(".state('crmman.sht_"+(String)ht.get("modid")+"', {");
	//		out.println("    url: '/sht_"+(String)ht.get("modid")+"',");
	//		out.println("    templateUrl: 'views/baseman/sht_view.jsp?shtid="+(String)ht.get("modtag")+"',");
	//		out.println("    data: {");
