 ...
 * @since 2019-06-20
 */
define(['module', 'controllerApi', 'base_obj_list', 'angular', 'swalApi', 'requestApi', '$q', 'openBizObj'], function (module, controllerApi, base_obj_list, angular, swalApi, requestApi, $q, openBizObj) {

	EpmReportList.$inject = ['$scope', '$stateParams', '$modal'];
	function EpmReportList(   $scope,   $stateParams,   $modal) {
        ...
        /* $scope.filterSetting.filters.audit_stat = {
            options: [
                { name: '新建', value: '新建' },
                { name: '一次报备提交', value: '一次报备提交' },
                { name: '一次报备通过', value: '一次报备通过' },
                { name: '一次报备拒绝', value: '一次报备拒绝' },
                { name: '二次报备提交', value: '二次报备提交' },
                { name: '二次报备通过', value: '二次报备通过' },
                { name: '二次报备拒绝', value: '二次报备拒绝' },
						...
						var modalInstance = $modal
							.openCommonSearch({
								...
								afterOk: function (proj) {
									return openBizObj({
										stateName: 'epmman.epm_strategy_report_prop',
										params: {
											reportAgain: true,
											project_id: proj.project_id
										}
