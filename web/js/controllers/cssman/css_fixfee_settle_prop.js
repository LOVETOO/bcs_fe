 ...
 * 维修费结算标准 css_fixfee_settle_prop
 */
//总金额=维修+信息+上门
define(
    ...
    function (module, controllerApi, base_obj_prop,swalApi,dateApi) {
        ...
        var controller = [
            ...
            function ($scope) {
                ...
                //计算总金额
                $scope.calculateNormAmt = function(){
                    if(!getCurrItem().way_fee)
                        getCurrItem().way_fee = 0;
                    if(!getCurrItem().info_fee)
                        getCurrItem().info_fee = 0;
                    if(!getCurrItem().fix_fee)
                        getCurrItem().fix_fee = 0;

                    getCurrItem().norm_amt = getCurrItem().way_fee+getCurrItem().info_fee+getCurrItem().fix_fee;
                };
