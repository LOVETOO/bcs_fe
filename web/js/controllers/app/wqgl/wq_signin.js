'use strict';
function Wq_Signin($scope,$rootScope,BasemanService,$state,localeStorageService){
    ...
    $scope.init_table = function(){
	    	$scope.FrmInfo={
			classid: 'app_wq_signin', //Search请求类---主表
			type: 'sqlback', //单选或多选，默认''单选
			sqlBlock: "signin_man != '该强制条件必须传对应字段属性所对应的值'", //强制性的过滤条件，默认空
			}
 ...
 // 查询
    function getSqlWhere(){
    	...
    	else	return BasemanService.getSqlWhere(["signin_man", "address", "mobile" ],$scope.searchtext);
    }
    $scope._pageLoad = function(postdata){
        ...
        $scope.highsql = "";
        BasemanService.RequestPost("app_wq_signin","search",postdata)
        .then(function(data){
            $scope.data.currItem.app_wq_signins =data.app_wq_signins;
             BasemanService.pageInfoOp($scope,data.pagination);
             $scope.data.currItem.app_wq_signins.length > 0?
...
//产品编辑页面
function Wq_Signin_Detail($scope, $http, $rootScope, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService) {
...
angular.module('inspinia')
    .controller('Wq_Signin', Wq_Signin)
    .controller('Wq_Signin_Detail', Wq_Signin_Detail)
