...
function edi_nolcquotaapplyinfoEdit($scope, $http, $rootScope, $modal, $location, $timeout, BasemanService, $state, localeStorageService, FormValidatorService, HistoryDataService, notify) {
    //继承基类方法
    edi_nolcquotaapplyinfoEdit = HczyCommon.extend(edi_nolcquotaapplyinfoEdit, ctrl_bill_public);
    edi_nolcquotaapplyinfoEdit.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    ...
    $scope.save_before = function () {
        ...
        delete $scope.data.currItem.edi_nolcquotaapplyinfo_flowofedi_lcquotaapplyinfos
        if($scope.data.currItem.lastpaymode1==0){
            $scope.data.currItem.lastpaymode1=""
        }
        if($scope.data.currItem.lastpaymode2==0){
            $scope.data.currItem.lastpaymode2=""
        }
        if($scope.data.currItem.lastpaymode3==0){
            $scope.data.currItem.lastpaymode3=""
        }

    }
    /**----------------------保存校验区域-------------------*/
    $scope.refresh_after = function () {
        $scope.data.currItem.contractpaymode=Number($scope.data.currItem.contractpaymode);
        $scope.data.currItem.lastpaymode1=Number($scope.data.currItem.lastpaymode1);
        $scope.data.currItem.lastpaymode2=Number($scope.data.currItem.lastpaymode2);
        $scope.data.currItem.lastpaymode3=Number($scope.data.currItem.lastpaymode3);
...
//加载控制器
billmanControllers
    .controller('edi_nolcquotaapplyinfoEdit', edi_nolcquotaapplyinfoEdit)
