            ...
            function ($scope,BasemanService,$timeout) {
                ...
                $scope.uncheck = function () {
                    ...
                    if($scope.data.currItem.last_year_month==undefined||$scope.data.currItem.last_year_month==''){
                        return swalApi.info("没有找到上一次月结的存货期间");
                    }
                ...
                $scope.doCheckYearMonth = function () {
                    var postData = {
                        classId: "inv_monthsum",
                        action: 'settle',
                        data: {year_month:$scope.data.currItem.year_month}
                    };
                    return requestApi.post(postData).then($scope.getYearMonth);
                }
                ...
                $scope.getYearMonth = function () {
                    var postData = {
                        classId: "inv_monthsum",
                        action: 'getsettlemonth',
                        data: {}
                    };
                    return requestApi.post(postData)
                        .then(function (data) {
                            $scope.data.currItem.year_month = data.year_month;
                            $scope.data.currItem.last_year_month = data.year_month_last;
