...
function sale_pi_m_headerEdit($scope, $http, $rootScope, $modal, $location, $timeout, BasemanService, $state, localeStorageService, FormValidatorService, HistoryDataService,notify) {
    //继承基类方法
    sale_pi_m_headerEdit = HczyCommon.extend(sale_pi_m_headerEdit, ctrl_bill_public);
    sale_pi_m_headerEdit.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
        name: "sale_pi_m_header",
        key: "pi_m_id",
        wftempid:10041,
        FrmInfo: {sqlBlock:" bill_flag =1"},		
        grids: [{optionname: 'options_11', idname: 'sale_pi_m_pay_lineofsale_pi_m_headers'},//资金预览
            {optionname: 'options_12', idname: 'sale_pi_m_feeofsale_pi_m_headers'},//费用明细
            {optionname: 'options_13', idname: 'sale_pi_m_box_lineofsale_pi_m_headers'},//柜型柜量
            {optionname: 'options_14', idname: 'sale_pi_m_money_lineofsale_pi_m_headers'},//款项明细
            {optionname: 'options_15', idname: 'sale_pi_m_area_attributeofsale_pi_m_headers'},//机型参数
            {optionname: 'options_16', idname: 'sale_pi_m_item_mbdataofsale_pi_m_headers'},//机型面板

            {
                optionname: 'options_21',
                idname: 'sale_pi_m_item_h_lineofsale_pi_m_headers',
                line: {optionname: "options_22", idname: "sale_pi_m_item_lineofsale_pi_m_item_h_lines"}
            },//订单明细  分体机
            {optionname: 'options_23', idname: 'sale_pi_m_item_partofsale_pi_m_headers'},//配件清单

            {optionname: 'options_31', idname: 'sale_pi_m_item_conf2ofsale_pi_m_headers'},//当前机型
            {optionname: 'options_32', idname: 'sale_pi_m_item_confofsale_pi_m_headers'},//所有机型
            {
                optionname: 'options_41',
                idname: 'sale_pi_m_break_headerofsale_pi_m_headers',
                line: {optionname: "options_42", idname: "sale_pi_m_break_lineofsale_pi_m_break_headers"}
            },//打散-包装方案  打散方案明细（左）
            {
                optionname: 'options_41',
                idname: 'sale_pi_m_break_headerofsale_pi_m_headers',
                line: {optionname: "options_43", idname: "sale_pi_m_break_itemofsale_pi_m_break_headers"}
            },//打散-包装方案  打散方案明细（右）
            {
                optionname: 'options_41',
                idname: 'sale_pi_m_break_headerofsale_pi_m_headers',
                line: {optionname: "options_44", idname: "sale_pi_m_break_itemallofsale_pi_m_break_headers"}
            },//打散-包装方案  包装方案
			{
                optionname: 'options_41',
                idname: 'sale_pi_m_break_headerofsale_pi_m_headers',
                line: {optionname: "options_45", idname: "sale_bill_break_itemofsale_pi_m_break_headers"}
            },//打散-包装方案  辅料用量汇总
            //{optionname: 'options_45', idname: 'sale_bill_break_item'},//辅料用量汇总
    ...
    $scope.change_org =function(){
		var i=0;
		alert($scope.data.currItem.org_name);
	}
	$scope.change_org1 =function(){
		var i=0;
		alert($scope.data.currItem.sales_user_id);
	}
	$scope.org_name =function(){
		alert($scope.data.currItem.org_name);
	}
	$scope.org_name1 =function(){
		alert($scope.data.currItem.sales_user_id);
	}
    /************************初始化页面***********************/
    ...
    $scope.validate = function () {
		...
		for(var i=0;i<nodes1.length;i++){
			if(parseInt(nodes1[i].data.line_type)!=4&&parseInt(nodes1[i].data.line_type)!=5&&parseInt(nodes1[i].data.line_type)!=8){
				if(parseFloat(nodes1[i].data.price||0)==0){
					BasemanService.notice("第"+(i+1)+"行类型为整机、内机、外机、CKD、SKD时，销售价不能为0","alert-warning");
			        return;
				}				
			}
        ...
        var errorlist = [];
        $scope.data.currItem.change_reason == undefined ? errorlist.push("变更原因为空") : 0;
	...
	$scope.getmax = function(data,string){
		var temp=0;
		for(var i=0;i<data.length;i++){
			if((parseInt(data[i][string])>temp)){
				temp=parseInt(data[i][string]);
			}
		}
		return temp;
	}
	...
	$scope.refresh_after= function(){
		...
		if($scope.options_41.api.getFocusedCell()){
            ...
            $scope.options_42.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[rowidx].sale_pi_m_break_lineofsale_pi_m_break_headers);			
			$scope.options_43.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[rowidx].sale_pi_m_break_itemofsale_pi_m_break_headers);
			$scope.options_44.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[rowidx].sale_pi_m_break_itemallofsale_pi_m_break_headers);
		}
		for(var i=0;i<$scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers.length;i++){
			$scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[i].queue=parseInt(i+1);
		}
		$scope.options_21.api.setRowData($scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers);
		var mystring = $scope.userbean.stringofrole;
        if(mystring.indexOf("销售人员") > -1&&mystring.indexOf("admin")<0){
		//业务结算价
	    $scope.options_21.columnApi.hideColumn('p4', true);
		$scope.options_22.columnApi.hideColumn('p4', true);
		//标准结算价
		$scope.options_21.columnApi.hideColumn('std_p4', true);
		$scope.options_22.columnApi.hideColumn('std_p4', true);
		//业务结算选配差价
		$scope.options_21.columnApi.hideColumn('c_price', true);
		$scope.options_22.columnApi.hideColumn('c_price', true);
		//标准选配结算差价
		$scope.options_21.columnApi.hideColumn('std_c_cl_cb', true);
		$scope.options_22.columnApi.hideColumn('std_c_cl_cb', true);
		}
		
	    if($scope.data.currItem.currprocname=="打散方案维护"){
			for(var i=0;i<$scope.columns_41.length;i++){
				if($scope.columns_41[i].field=="break_name"||$scope.columns_41[i].field=="item_type_name"){
		...
		var temp=$scope.options_21.api.getModel().rootNode.childrenAfterSort[0].data;
		var colmun=$scope.options_22.columnApi.getAllColumns();
		if(temp.line_type!=6&&temp.line_type!=7){
				 for(var i=0;i<colmun.length;i++){
					 if(colmun[i].colDef.field=="sj_type"){
	...
	$scope.authority =function(){
		$timeout(function(){
			$("#myid2").attr('disabled', true).trigger("chosen:updated");
			//常规信息变更
			if($scope.data.currItem.a_change==2){
			  $("#contract_subscrp").attr('readonly', false).trigger("chosen:updated");
              $("#brand_name").attr('disabled', false).trigger("chosen:updated");			  
			}else{
			  $("#contract_subscrp").attr('readonly', true).trigger("chosen:updated");	
			  $("#brand_name").attr('disabled', true).trigger("chosen:updated");
			}
			//付款方式变更
			if($scope.data.currItem.b_change==2){
			  $("#myid1").attr('disabled', false).trigger("chosen:updated");	
			}else{
			  $("#myid1").attr('disabled', true).trigger("chosen:updated");	
			}
			//交期(港口)变更
			if($scope.data.currItem.c_change==2){
			  $("#pre_ship_date").attr('disabled', false).trigger("chosen:updated");
              $("#latest_ship_date").attr('disabled', false).trigger("chosen:updated");
			  $("#commission_rate").attr('readonly', false).trigger("chosen:updated");
			  $("#rebate_rate").attr('readonly', false).trigger("chosen:updated");
			  $("#material_rate").attr('readonly', false).trigger("chosen:updated");
			  
			}else{
			  $("#pre_ship_date").attr('disabled', true).trigger("chosen:updated");
              $("#latest_ship_date").attr('disabled', true).trigger("chosen:updated");	
              $("#commission_rate").attr('readonly', true).trigger("chosen:updated");
              $("#rebate_rate").attr('readonly', true).trigger("chosen:updated");	
              $("#material_rate").attr('readonly', true).trigger("chosen:updated");			  
			}
			//有效期更改
			if($scope.data.currItem.valid_flag==2){
			  $("#valid_date").attr('disabled', false).trigger("chosen:updated");
			}else{
			  $("#valid_date").attr('disabled', true).trigger("chosen:updated");
			}
			
			//柜型变更
			if($scope.data.currItem.g_change==2){
			   var colmun=$scope.options_13.defaultColumns;
			   for(var i=0;i<$scope.columns_13.length;i++){
				 $scope.columns_13[i].editable=colmun[i].editable;
			   }
			}else{
			  var colmun=$scope.options_13.columnApi.getAllColumns();
			  for(var i=0;i<colmun.length;i++){
				  colmun[i].colDef.editable=false;
			  }
			}
			//数量变更
			if($scope.data.currItem.d_change==2){
			   var colmun=$scope.options_21.columnApi.getAllColumns();
			   for(var i=0;i<colmun.length;i++){
				 if(colmun[i].colId=="qty"){
					 colmun[i].colDef.editable=true;
				 }
			   }
			}else{
            ...
            //配件比例变更
			if($scope.data.currItem.j_change==2){
			  $("#part_rate").attr('readonly', false).trigger("chosen:updated");	
			}else{
			  $("#part_rate").attr('readonly', true).trigger("chosen:updated");	
			}
			//价格条款变更
			if($scope.data.currItem.f_change==2){
			  $("#price_type_id").attr('disabled', false).trigger("chosen:updated");	
			}else{
			  $("#price_type_id").attr('disabled', true).trigger("chosen:updated");	
			}
			//价格相关变更
            if($scope.data.currItem.e_change==2){
			   var colmun=$scope.options_21.columnApi.getAllColumns();
			   for(var i=0;i<colmun.length;i++){
				 if(colmun[i].colId=="p4"||colmun[i].colId=="c_price"||colmun[i].colId=="price"){
					 colmun[i].colDef.editable=true;
				 }
			   }
			}else{
            ...
            //增加机型或配件
            if($scope.data.currItem.i_change==2||$scope.data.currItem.l_change==2){
			   var colmun=$scope.options_21.columnApi.getAllColumns();
			   for(var i=0;i<colmun.length;i++){
				 if(colmun[i].colId=="item_h_code"){
					 colmun[i].colDef.editable=true;
				 }
			   }
			}else{
	...
	$scope.selectpi_no =function(){
		...
		$scope.FrmInfo = {
			...
			sqlBlock:" stat=5 and not exists (select 1 from sale_pi_m_header mh where mh.pi_id=Sale_Pi_Header.pi_id and mh.stat < 5 ) and nvl(bill_flag,0) not in ( 2,3) "
        };
        BasemanService.open(CommonPopController, $scope)
            .result.then(function (result) {
            $scope.data.currItem.pi_id=parseInt(result.pi_id);
			BasemanService.RequestPost("sale_pi_header", "select", {pi_id: $scope.data.currItem.pi_id}).then(function (data) {				
				$scope.data.currItem=data;
				for(name in $scope.data.currItem){
				var name1=name.replace(/sale_pi_/g,"sale_pi_m_");
							...
							for(var j=0;j<$scope.data.currItem[name1].length;j++){
							    for(name2 in $scope.data.currItem[name1][j]){
								  if($scope.data.currItem[name1][j][name2] instanceof Array){
									  var name3=name2.replace(/sale_pi_/g,"sale_pi_m_");
				...
				$scope.data.currItem.old_guide_hwmlv=$scope.data.currItem.guide_hwmlv;
				$scope.data.currItem.old_zb_profit_rate=$scope.data.currItem.zb_profit_rate;
				$scope.data.currItem.old_std_jzlmll=$scope.data.currItem.std_jzlmll;
				$scope.data.currItem.old_guide_hwmlv2=$scope.data.currItem.guide_hwmlv2;
				$scope.data.currItem.sale_pi_m_headers=$scope.data.currItem.sale_pi_m_m_headers;
				$scope.data.currItem.cust_level=parseInt($scope.data.currItem.cust_level);
				//$scope.data.currItem.version="";
		        $scope.data.currItem.box_send=parseInt($scope.data.currItem.box_send);
				delete $scope.data.currItem.sale_pi_m_m_headers;
				$scope.data.brandlist=$scope.data.currItem.customer_brandofcustomers;
				$scope.data.currItem.stat=1;	
                $scope.data.currItem.pi_m_no="";				
				$scope.options_11.api.setRowData($scope.data.currItem.sale_pi_m_pay_lineofsale_pi_m_headers); //资金预览
				$scope.options_12.api.setRowData($scope.data.currItem.sale_pi_m_feeofsale_pi_m_headers);//费用明细
				$scope.options_13.api.setRowData($scope.data.currItem.sale_pi_m_box_lineofsale_pi_m_headers);//柜型柜量
				$scope.options_14.api.setRowData($scope.data.currItem.sale_pi_m_money_lineofsale_pi_m_headers);//款项明细
				$scope.options_15.api.setRowData($scope.data.currItem.sale_pi_m_area_attributeofsale_pi_m_headers);//机型参数
				$scope.options_16.api.setRowData($scope.data.currItem.sale_pi_m_item_mbdataofsale_pi_m_headers);//机型面板
				
				$scope.options_21.api.setRowData($scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers);//订单明细 
				$scope.options_22.api.setRowData($scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[0].sale_pi_m_item_lineofsale_pi_m_item_h_lines);//分体机
				 
                $scope.options_23.api.setRowData($scope.data.currItem.sale_pi_m_item_partofsale_pi_m_headers);//配件清单
                $scope.options_31.api.setRowData($scope.data.currItem.sale_pi_m_item_conf2ofsale_pi_m_headers);//当前机型
				$scope.options_32.api.setRowData($scope.data.currItem.sale_pi_m_item_confofsale_pi_m_headers);//所有机型
				
				$scope.options_41.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers);
				//$scope.options_42.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[0].sale_pi_m_break_lineofsale_pi_m_break_headers)//打散-包装方案  打散方案明细（左）
				
				$scope.options_41.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers);
				//$scope.options_43.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[0].sale_pi_m_break_itemofsale_pi_m_break_headers)//打散-包装方案  打散方案明细（右）
				
				$scope.options_41.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers);
				//$scope.options_44.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[0].sale_pi_m_break_itemallofsale_pi_m_break_headers)//
				
				$scope.options_41.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers);
				///$scope.options_45.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[0].sale_bill_break_itemofsale_pi_m_break_headers)
	...
	$scope.confirm1 =function(){
		$scope.data.currItem.modify_flah=2;
		$scope.authority();
	}
	$scope.psy =function(){
		$scope.FrmInfo = {
        ...
        BasemanService.open(CommonPopController, $scope)
            .result.then(function (result) {
            $scope.data.currItem.psy = result.username;
        });
	}
    // 到货港
...
//            data[index].apply_price = parseFloat(items.price);
			...
			if(items.superid==undefined){
				 data[index].superid=0;
			}else{
				data[index].superid = parseInt(items.superid);
			}
            
            data[index].seq = parseInt(node[index].data.seq);
            data[index].cool_stand=parseInt(items.cool_stand);
			data[index].mb_stand=parseInt(items.mb_stand);
			if(items.hitem_h_id==undefined){
				 data[index].hitem_h_id=0;
			}else{
				data[index].hitem_h_id = parseInt(items.hitem_h_id);
			}
			if(items.hitem_type_id==undefined){
				 data[index].hitem_type_id=0;
			}else{
				data[index].hitem_type_id = parseInt(items.hitem_type_id);
			}
			var myDate=new Date();
			data[index].price_time=myDate.toLocaleDateString();
            $scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers = data;
            $scope.options_21.api.setRowData(data);
			$scope.options_31.api.setRowData([]);
            // 查询分体机(业务逻辑操作)
            $scope.proItem(data[index].pro_type, parseInt(index), parseInt(data[index].item_h_id));
			
	    var temp=$scope.options_21.api.getModel().rootNode.childrenAfterSort[$scope.options_21.api.getFocusedCell().rowIndex].data;
		var colmun=$scope.options_22.columnApi.getAllColumns();
		if(temp.line_type!=6&&temp.line_type!=7){
				 for(var i=0;i<colmun.length;i++){
					 if(colmun[i].colDef.field=="sj_type"){
    ...
    $scope.break_name41 = function () {
        ...
        BasemanService.open(CommonPopController, $scope)
            ...
            $scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers = data;
            ...
            promise.then(function (data) {
			$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[index].sale_pi_m_break_lineofsale_pi_m_break_headers=data.sale_break_lineofsale_break_headers;
			$scope.options_42.api.setRowData(data.sale_break_lineofsale_break_headers);
			
			$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[index].sale_pi_m_break_itemofsale_pi_m_break_headers=[];
			$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[index].sale_pi_m_break_itemallofsale_pi_m_break_headers=[];
			$scope.options_43.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[index].sale_pi_m_break_itemofsale_pi_m_break_headers);
			});
    ...
    $scope.cellchange41 = function () {
            ...
            if(val==""){
			$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[index].sale_pi_m_break_lineofsale_pi_m_break_headers=[];	
			$scope.options_42.api.setRowData([]);
			$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[index].sale_pi_m_break_itemofsale_pi_m_break_headers=[];
			$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[index].sale_pi_m_break_itemallofsale_pi_m_break_headers=[];
			$scope.options_43.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[index].sale_pi_m_break_itemofsale_pi_m_break_headers);
			$scope.options_44.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[index].sale_pi_m_break_itemallofsale_pi_m_break_headers);
			}		
    }
	$scope.amount11 =function(){
		var _this = $(this);	var val = _this.val();
		var index = $scope.options_11.api.getFocusedCell().rowIndex;
		var nodes=$scope.options_11.api.getModel().rootNode.childrenAfterSort;
		nodes[index].data.pay_ratio=(parseFloat(val||0)/parseFloat($scope.data.currItem.amt_total||0.0000000000000000001)*100).toFixed(2);
		$scope.options_11.api.refreshCells(nodes,["pay_ratio"]);
	...
	$scope.item_code43 = function () {
        ...
        BasemanService.open(CommonPopController, $scope)
			...
			$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[rowidx].sale_pi_m_break_itemofsale_pi_m_break_headers=data;
		    $scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[rowidx].sale_pi_m_break_itemallofsale_pi_m_break_headers=data;
    ...
    $scope.proItem = function (pro_type, index, item_h_id) {
        ...
        promise.then(function (data) {
            for (var i = 0; i < data.pro_items.length; i++) {
                ...
                data.pro_items[i].qty = $scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[index].qty;
                data.pro_items[i].cust_item_name = $scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[index].cust_item_name;
				data.pro_items[i].cust_spec=data.pro_items[i].item_name;
				
            }
            $scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[index].sale_pi_m_item_lineofsale_pi_m_item_h_lines = data.pro_items;
            $scope.options_22.api.setRowData($scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[index].sale_pi_m_item_lineofsale_pi_m_item_h_lines);
            var l = {};
			//传参无需改变容器
            l.sale_pi_item_h_lineofsale_pi_headers = [];
            l.sale_pi_item_h_lineofsale_pi_headers.push($scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[index]);
			l.sale_pi_item_h_lineofsale_pi_headers[0].sale_pi_item_lineofsale_pi_item_h_lines=l.sale_pi_item_h_lineofsale_pi_headers[0].sale_pi_m_item_lineofsale_pi_m_item_h_lines;
			delete l.sale_pi_item_h_lineofsale_pi_headers[0].sale_pi_m_item_lineofsale_pi_m_item_h_lines;
            ...
            $scope.selectprice(postdata, pro_type, $scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[index], index);
			
        })
    }
	//定单类型改变，清空所有网格数据
    $scope.sale_order_type =function(){
		$scope.data.currItem.sale_pi_m_pay_lineofsale_pi_m_headers=[];
		$scope.options_11.api.setRowData([])
		$scope.data.currItem.sale_pi_m_feeofsale_pi_m_headers=[];
		$scope.options_12.api.setRowData([])
		$scope.data.currItem.sale_pi_m_box_lineofsale_pi_m_headers=[];
		$scope.options_13.api.setRowData([])
		$scope.data.currItem.sale_pi_m_money_lineofsale_pi_m_headers=[];
		$scope.options_14.api.setRowData([])
		$scope.data.currItem.sale_pi_m_area_attributeofsale_pi_m_headers=[];
		$scope.options_15.api.setRowData([])
		$scope.data.currItem.sale_pi_m_item_mbdataofsale_pi_m_headers=[];
		$scope.options_16.api.setRowData([])
		
		$scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers=[];
		$scope.options_21.api.setRowData($scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers);
		$scope.options_22.api.setRowData([]);
		$scope.data.currItem.sale_pi_m_item_partofsale_pi_m_headers=[];
		$scope.options_23.api.setRowData([]);
		
		$scope.data.currItem.sale_pi_m_item_conf2ofsale_pi_m_headers=[];
		$scope.options_31.api.setRowData($scope.data.currItem.sale_pi_m_item_conf2ofsale_pi_m_headers);
		$scope.data.currItem.sale_pi_m_item_confofsale_pi_m_headers=[];
		$scope.options_32.api.setRowData($scope.data.currItem.sale_pi_m_item_confofsale_pi_m_headers);
		
		$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers=[];
		$scope.options_41.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers);
		$scope.options_42.api.setRowData([]);
		$scope.options_43.api.setRowData([]);
		$scope.options_44.api.setRowData([]);
		$scope.options_45.api.setRowData([]);
		$scope.options_46.api.setRowData([]);
		$scope.options_47.api.setRowData([]);		
	}
    ...
    /**$scope.selectprice = function (postdata, line_type, dataheader,index) {
        if (line_type != 4) {
            ...
            promise.then(function (data) {
                for (var i = 0; i < data.sale_pi_m_item_h_lineofsale_pi_m_headers.length; i++) {
                    //内销订单要收取汇率费用
                    if ($scope.data.currItem.sale_order_type == 3) {
                        //订单明细
                        dataheader.guide_p6 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].guide_p6) * 1.17);
                        dataheader.p4 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].p4) * 1.17);
                        dataheader.std_p4 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].std_p4) * 1.17);
                        dataheader.make_p4 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].make_p4) * 1.17);
                        dataheader.p6 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].p6) * 1.17);
                        dataheader.std_p6 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].std_p6) * 1.17);
                        dataheader.guide_p4 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].guide_p4) * 1.17);
                        //分体机
                        var dataline = data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].sale_pi_m_item_lineofsale_pi_m_item_h_lines;
                        for (var j = 0; j < dataline.length; j++) {
                            ...
                            dataline[j].p6 = HczyCommon.toDecimal2(dataline[j].p6 * 1.17)
                            dataline[j].std_p6 = HczyCommon.toDecimal2(dataline[j].std_p6 * 1.17)
                            dataline[j].guide_p4 = HczyCommon.toDecimal2(dataline[j].guide_p4 * 1.17)
                        }
                    } else {
                        //订单明细
                        dataheader.guide_p6 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].guide_p6));
                        dataheader.p4 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].p4));
                        dataheader.std_p4 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].std_p4));
                        dataheader.make_p4 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].make_p4));
                        dataheader.p6 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].p6));
                        dataheader.std_p6 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].std_p6));
                        dataheader.guide_p4 = HczyCommon.toDecimal2(Number(data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].guide_p4));
                        //分体机
                        var dataline = data.sale_pi_m_item_h_lineofsale_pi_m_headers[i].sale_pi_m_item_lineofsale_pi_m_item_h_lines;
                        for (var j = 0; j < dataline.length; j++) {
                            ...
                            dataline[j].p6 = HczyCommon.toDecimal2(dataline[j].p6)
                            dataline[j].std_p6 = HczyCommon.toDecimal2(dataline[j].std_p6)
                            dataline[j].guide_p4 = HczyCommon.toDecimal2(dataline[j].guide_p4)
                        }

                    }
                }
                $scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[index] = dataheader;
                $scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[index].sale_pi_m_item_lineofsale_pi_m_item_h_lines = dataline;
            ...
            promise.then(function (data) {
                for (var i = 0; i < data.sale_pi_item_h_lineofsale_pi_headers.length; i++) {
                    ...
                    if ($scope.data.currItem.sale_order_type == 3) {
                        ...
                        dataheader.p6 = HczyCommon.toDecimal2(Number(data.sale_pi_item_h_lineofsale_pi_headers[i].p6) * 1.17);
                        dataheader.std_p6 = HczyCommon.toDecimal2(Number(data.sale_pi_item_h_lineofsale_pi_headers[i].std_p6) * 1.17);
                        dataheader.guide_p4 = HczyCommon.toDecimal2(Number(data.sale_pi_item_h_lineofsale_pi_headers[i].guide_p4) * 1.17);
                        //分体机
                        ...
                        dataheader.p6 = HczyCommon.toDecimal2(Number(data.sale_pi_item_h_lineofsale_pi_headers[i].p6));
                        dataheader.std_p6 = HczyCommon.toDecimal2(Number(data.sale_pi_item_h_lineofsale_pi_headers[i].std_p6));
                        dataheader.guide_p4 = HczyCommon.toDecimal2(Number(data.sale_pi_item_h_lineofsale_pi_headers[i].guide_p4));
                        //分体机
                        var dataline = data.sale_pi_item_h_lineofsale_pi_headers[i].sale_pi_item_lineofsale_pi_item_h_lines;
                        for (var j = 0; j < dataline.length; j++) {
                            dataline[j].guide_p6 = HczyCommon.toDecimal2(dataline[j].guide_p6)
	...
	$scope.computeqty = function (qty) {
        ...
        var pi_item_h_lines = itemtemp.sale_pi_m_item_lineofsale_pi_m_item_h_lines;
        var qty_temp = 0;
        var sum_qty = 0;
        var sum_amt = 0;
    ...
    $scope.getoneYS = function (sale_amt, items) {
        ...
        if (items[0].sale_pi_item_h_lineofsale_pi_headers.length) {
            ...
            object.sale_pi_m_item_lineofsale_pi_m_item_h_lines=object.sale_pi_item_lineofsale_pi_item_h_lines;
			delete object.sale_pi_item_lineofsale_pi_item_h_lines;
            object.sale_pi_m_item_lineofsale_pi_m_item_h_lines[0].qty = 1;
            object.sale_pi_m_item_lineofsale_pi_m_item_h_lines[0].line_type = 4;
            object.sale_pi_m_item_lineofsale_pi_m_item_h_lines[0].sale_amt = parseFloat(sale_amt || 0);
            object.sale_pi_m_item_lineofsale_pi_m_item_h_lines[0].price = parseFloat(sale_amt || 0);
        }
        return object;
    }
    //递归方法 循环递归处理树状数据结构
    ...
    $scope.openCustPriceType = function () {
        if ($scope.data.currItem.customer_payment_typeofcustomers.length > 0) {
			if($scope.data.currItem.payment_type_id){
				...
				for(var i=0;i<$scope.data.currItem.customer_payment_typeofcustomers.length;i++){
					if($scope.data.currItem.payment_type_code==$scope.data.currItem.customer_payment_typeofcustomers[i].payment_type_code){
						var payment_type_id=$scope.data.currItem.customer_payment_typeofcustomers[i].payment_type_id
            ...
            BasemanService.RequestPost("customer", "getpayline", postdata)
                .then(function (data) {
                    $scope.data.currItem.sale_pi_m_pay_lineofsale_pi_m_headers = HczyCommon.stringPropToNum(data.customer_paytype_detailofcustomers);
					$scope.options_11.api.setRowData($scope.data.currItem.sale_pi_m_pay_lineofsale_pi_m_headers);
    ...
    $scope.rowClicked21 = function (event) {
		...
		var temp=$scope.options_21.api.getModel().rootNode.childrenAfterSort[$scope.options_21.api.getFocusedCell().rowIndex].data;
		if(temp.line_type==4){
            if(temp.queue!=undefined){
				for(var i=0;i<colmun.length;i++){
				if((colmun[i].colDef.editable==true&&colmun[i].colDef.field!="cust_item_name")&&(colmun[i].colDef.editable==true&&colmun[i].colDef.field!="brand_name")){
					colmun[i].colDef.editable=false;
        ...
        //分体机==
        if (event.data) {
            if (event.data.sale_pi_m_item_lineofsale_pi_m_item_h_lines == undefined) {
                event.data.sale_pi_m_item_lineofsale_pi_m_item_h_lines = [];
            }
			if($scope.options_22.api.getFocusedCell()){
				...
				$scope.options_22.api.setRowData(event.data.sale_pi_m_item_lineofsale_pi_m_item_h_lines);	
			    }	
			}else{
				$scope.options_22.api.setRowData(event.data.sale_pi_m_item_lineofsale_pi_m_item_h_lines);
			}
    ...
    $scope.rowClicked41 = function (event) {
        if (event.data) {
            //关联打散方案明细左
            if (event.data.sale_pi_m_break_lineofsale_pi_m_break_headers == undefined) {
                event.data.sale_pi_m_break_lineofsale_pi_m_break_headers = [];
            }
            $scope.options_42.api.setRowData(event.data.sale_pi_m_break_lineofsale_pi_m_break_headers);
            //关联打散方案明细右
            if (event.data.sale_pi_m_break_itemofsale_pi_m_break_headers == undefined) {
                event.data.sale_pi_m_break_itemofsale_pi_m_break_headers = [];
            }
            $scope.options_43.api.setRowData(event.data.sale_pi_m_break_itemofsale_pi_m_break_headers);
            //关联包装方案
            if (event.data.sale_pi_m_break_itemallofsale_pi_m_break_headers == undefined) {
                event.data.sale_pi_m_break_itemallofsale_pi_m_break_headers = [];
            }
            $scope.options_44.api.setRowData(event.data.sale_pi_m_break_itemallofsale_pi_m_break_headers);
			//辅料用量汇总
            if (event.data.sale_bill_break_itemofsale_pi_m_break_headers == undefined) {
                event.data.sale_bill_break_itemofsale_pi_m_break_headers = [];
            }
            $scope.options_45.api.setRowData(event.data.sale_bill_break_itemofsale_pi_m_break_headers);
    ...
    $scope.cust_item_name21 = function () {
        ...
        var nodes = $scope.options_21.api.getModel().rootNode.childrenAfterSort;
       if (index != undefined) {
            for (var i = 0; i < nodes[index].data.sale_pi_m_item_lineofsale_pi_m_item_h_lines.length; i++) {
                nodes[index].data.sale_pi_m_item_lineofsale_pi_m_item_h_lines[i].cust_item_name = val;
            }
        }
        $scope.options_22.api.setRowData(nodes[index].data.sale_pi_m_item_lineofsale_pi_m_item_h_lines);
		
		if(parseInt(nodes[index].data.line_type)==4){
			var data=$scope.gridGetData("options_23");
			for(var i=0;i<data.length;i++){
				if(parseInt(data[i].item_line_seq)==parseInt(nodes[index].data.seq)){
					data[i].cust_item_name=val;
				}
			}
    ...
    $scope.qty21 = function () {
        ...
        var nodes = $scope.options_21.api.getModel().rootNode.childrenAfterSort;
        var itemtemp = nodes[index].data;
		var before=nodes[index].data.sale_amt;
        var pi_item_h_lines = itemtemp.sale_pi_m_item_lineofsale_pi_m_item_h_lines;
        ...
        itemtemp.sale_amt = parseFloat(val || 0) * parseFloat(itemtemp.price);
        $scope.options_21.api.refreshCells(nodes, ["sale_amt"]);
        $scope.data.currItem.amt_total=parseFloat($scope.data.currItem.amt_total)+itemtemp.sale_amt-parseFloat(before);
        if (index != undefined) {
            val = val ? val : 0;
            for (var i = 0; i < pi_item_h_lines.length; i++) {
                if (pi_item_h_lines.length > 0) {
    ...
    $scope.addline21 = function () {
		...
		if($scope.data.currItem.l_change==2){
			var item = {seq: 1 + max, line_type: 4, brand_name: brand_id};
		}else{
			var item = {seq: 1 + max, line_type: 1, brand_name: brand_id};
		}
    ...
    $scope.delline21 = function () {
        ...
        var data1=$scope.gridGetData("options_23");
	    if(parseInt(data[rowidx].line_type)==4){			
		    for(var i=data1.length-1;i>=0;i--){
			    if(parseInt(data1[i].item_line_seq||0)==parseInt(data[rowidx].seq)){
					data1.splice(i,1);
				}
		    }
		};		
		...
		if(rowidx>0){
		  ...
		  $scope.options_22.api.setRowData($scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[rowidx].sale_pi_m_item_lineofsale_pi_m_item_h_lines);		  
		}else{
		  ...
		  var colKey= $scope.options_21.api.getFocusedCell().column.colId
		  $scope.options_21.api.setFocusedCell(rowidx,colKey);
		  $scope.options_22.api.setRowData($scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[rowidx+1].sale_pi_m_item_lineofsale_pi_m_item_h_lines);	  
		  }
		}		
		//删除配件行
		$scope.options_23.api.setRowData(data1);
    }
    //订单明细 
	$scope.copycountry =function(){
		var data=$scope.gridGetData("options_21");
		var index=$scope.options_21.api.getFocusedCell().rowIndex;
		for(var i=0;i<data.length;i++){
			if(i!=index){
				data[i].area_names=data[index].area_names;
				data[i].area_ids=data[index].area_ids;
			}
		}
		$scope.options_21.api.setRowData(data);
	}
    ...
    $scope.addpro_parts = function () {
        ...
        BasemanService.openFrm("views/saleman/part_additem.html", part_additem, $scope, "", "lg")
            ...
            var max=$scope.getmax(data1,"seq");
			var cust_item_name=data1[index].cust_item_name;
            //新增配件明细
            if (items[1].pro_item_partofpro_items.length) {
                if ($scope.data.currItem.sale_order_type != 3) {
                    ...
                    for (var i = 0; i < items[1].pro_item_partofpro_items.length; i++) {
						...
						items[1].pro_item_partofpro_items[i].item_line_seq = max+1;
						items[1].pro_item_partofpro_items[i].cust_item_name = cust_item_name;
            ...
            $scope.options_23.api.setRowData(data);
				...
				temp.cust_item_name=cust_item_name;
                temp.seq = (1 + max);
                data1.push(temp);
	...
	//订单明细 复制历史机型
	$scope.copycuitem =function(){
		var data=$scope.gridGetData("options_21");
		var index=$scope.options_21.api.getFocusedCell().rowIndex;
		var object={};
		HczyCommon.copyobj(data[index],object)
		object.pi_m_h_id=0;
		data.push(object);
		for(var i=0;i<object.sale_pi_m_item_lineofsale_pi_m_item_h_lines.length;i++){
			object.sale_pi_m_item_lineofsale_pi_m_item_h_lines[i].pi_line_id=0;
		}
		$scope.options_21.api.setRowData(data);
	}
	...
	$scope.setresult=function(data,result){
		...
		for(var j=0;j<result.length;j++){
              ...
              re.sale_pi_item_h_lineofsale_pi_headers[0].sale_pi_m_item_lineofsale_pi_m_item_h_lines=re.sale_pi_item_lineofsale_pi_headers
			  data.push(re.sale_pi_item_h_lineofsale_pi_headers[0]); 
              $scope.options_21.api.setRowData(data);			  
              })
			}
	}
...
function fnPIconfBgClose() {
    ...
    $scope.addline23 = function () {
		if($scope.options_21.api.getFocusedCell()){
			var index =$scope.options_21.api.getFocusedCell().rowIndex;
			var data1=$scope.gridGetData("options_21");
			if(parseInt(data1[index].line_type)==4){
				var temp=data1[index]
			}else{
			BasemanService.notice("请指定明细的机型配件", "alert-warning");
            return;
			}
		}else{
			BasemanService.notice("请指定明细的机型配件", "alert-warning");
        ...
        BasemanService.openFrm("views/saleman/part_additem.html", part_additem, $scope, "", "lg")
            ...
            if (items[1].pro_item_partofpro_items.length) {
                if ($scope.data.currItem.sale_order_type != 3) {
                    ...
                    for (var i = 0; i < items[1].pro_item_partofpro_items.length; i++) {
                        items[1].pro_item_partofpro_items[i].cust_sett_price = parseFloat(items[1].pro_item_partofpro_items[i].price);
						items[1].pro_item_partofpro_items[i].item_line_seq=temp.seq;
						items[1].pro_item_partofpro_items[i].cust_item_name=temp.cust_item_name;
                        data.push(items[1].pro_item_partofpro_items[i]);
                        //将其金额汇总
                        sale_amt += parseFloat(items[1].pro_item_partofpro_items[i].amt || 0);
                    }
               ...
               $scope.options_23.api.setRowData(data);
                //返回汇总行
                var temp = $scope.getoneYS(sale_amt, items);
                data1[index].sale_amt = parseFloat(data1[index].sale_amt || 0) + temp.sale_amt;
                data1[index].price = parseFloat(data1[index].price || 0) + temp.price;
                data1[index].sale_pi_m_item_lineofsale_pi_m_item_h_lines[0].sale_amt = parseFloat(data1[index].sale_pi_m_item_lineofsale_pi_m_item_h_lines[0].sale_amt || 0)
                + temp.sale_pi_m_item_lineofsale_pi_m_item_h_lines[0].sale_amt;
                data1[index].sale_pi_m_item_lineofsale_pi_m_item_h_lines[0].price = parseFloat(data1[index].sale_pi_m_item_lineofsale_pi_m_item_h_lines[0].price || 0)
                + temp.sale_pi_m_item_lineofsale_pi_m_item_h_lines[0].price;
    ...
    $scope.addline42 = function () {
        ...
        data.push(item);
		var rowidx = $scope.options_41.api.getFocusedCell().rowIndex;
		$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[rowidx].sale_pi_m_break_lineofsale_pi_m_break_headers=data;
    ...
    $scope.synchronize42 = function () {
        ...
        postdata.pi_line_id = node[rowidx].data.pi_line_id;
        postdata.pi_id = $scope.data.currItem.pi_id;
        var promise = BasemanService.RequestPost("sale_pi_header", "synbreak", postdata);
        promise.then(function (data) {
    ...
    $scope.generate42 = function () {
        if (!$scope.options_41.api.getFocusedCell()) {
            BasemanService.notice("未选中要生成的方案明细", "alert-info");
            return;
        }
        var rowidx = $scope.options_41.api.getFocusedCell().rowIndex;
        var node = $scope.options_41.api.getModel().rootNode.childrenAfterSort;
        ...
        var promise = BasemanService.RequestPost("sale_pi_header", "getsparebom", postdata);
        promise.then(function (data) {
            //生成打散方案明细(右)
	...
	$scope.ptype_name = function () {
        ...
        BasemanService.open(CommonPopController, $scope)
			...
			$scope.data.currItem.ptype_id = result.pack_id;	
	...
	$scope.ptype_nameall = function () {
		...
		for(var i=0;i<$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers.length;i++){
			for(var j=0;j<$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[i].sale_pi_m_break_itemofsale_pi_m_break_headers.length;j++){
			$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[i].sale_pi_m_break_itemofsale_pi_m_break_headers[j].ptype_name=$scope.data.currItem.ptype_name;
			$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[i].sale_pi_m_break_itemofsale_pi_m_break_headers[j].ptype_id=$scope.data.currItem.ptype_id;
			$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[i].sale_pi_m_break_itemallofsale_pi_m_break_headers[j].ptype_name=$scope.data.currItem.ptype_name;
			$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[i].sale_pi_m_break_itemallofsale_pi_m_break_headers[j].ptype_id=$scope.data.currItem.ptype_id;
			}			
		}               		
        $scope.options_43.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[rowidx].sale_pi_m_break_itemofsale_pi_m_break_headers);			
    }
    ...
    $scope.generate43 = function () {
        ...
        var node = $scope.options_41.api.getModel().rootNode.childrenAfterSort;
        var postdata = {};
		//postdata.line_id=node[rowidx].data.pi_line_id;
        postdata.pi_line_id = node[rowidx].data.pi_line_id;
        postdata.pi_id = $scope.data.currItem.pi_id;
        postdata.pi_h_id = node[rowidx].data.pi_h_id;
    ...
    //打散-包装方案 打散方案明细(右) 方案类型复制到相同编码机型
	 $scope.copyitem43 = function () {
		...
		for(var i=0;i<$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers.length;i++){
			for(var j=0;j<$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[i].sale_pi_m_break_itemofsale_pi_m_break_headers.length;j++){
				if($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[i].sale_pi_m_break_itemofsale_pi_m_break_headers[j].item_code==nodes[rowidx].data.item_code){
				$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[i].sale_pi_m_break_itemofsale_pi_m_break_headers[j].ptype_name=nodes[rowidx].data.ptype_name;
			     }
			}
		}
        $scope.options_43.api.setRowData($scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[rowidx1].sale_pi_m_break_itemofsale_pi_m_break_headers);
	 }
    //打散-包装方案 包装方案 增加行
    ...
    $scope.get44 = function () {
        ...
        var promise = BasemanService.RequestPost("sale_pi_header", "matchpackage", postdata);
        promise.then(function (data) {
	...
	$scope.cancel44 = function () {
		var data1=[];
		var node = $scope.options_44.api.getModel().rootNode.childrenAfterSort;
		var data = $scope.options_44.api.getSelectedRows();
		if(data.length==0){
		...
		data1.push(data[0]);
		var rowidx = $scope.options_41.api.getFocusedCell().rowIndex;
		$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[rowidx].sale_pi_m_break_itemallofsale_pi_m_break_headers=data1;
	    $scope.options_44.api.setRowData(data1);
    }
    //打散-包装方案 包装方案 计算包装方案
    $scope.compute44 = function () {
        postdata = {};
        postdata.pi_id = $scope.data.currItem.pi_id;
        var promise = BasemanService.RequestPost("sale_pi_header", "calpackagefee", postdata);
        promise.then(function (data) {
            $scope.refresh(2);
    ...
    $scope.getpack = function () {
		...
		var node=$scope.options_41.api.getModel().rootNode.childrenAfterSort;
		for (var i = 0; i < node.length; i++) {
			for(var j=0;j<node[i].data.sale_pi_m_break_itemallofsale_pi_m_break_headers.length;j++)
            if (node[i].data.sale_pi_m_break_itemallofsale_pi_m_break_headers[j].package_name == undefined||node[i].data.sale_pi_m_break_itemallofsale_pi_m_break_headers[j].package_name=="") {
                data.push(node[i].data.sale_pi_m_break_itemallofsale_pi_m_break_headers[j]);
            }
        }
    ...
    $scope.item_code46 = function () {
        ...
        promise.then(function (data) {
            ...
            shift[0].children = [];
            //循环递归处理树状数据结构
            //$scope.options_46.api.setRowData(data.sale_pi_m_item_h_lineofsale_pi_m_headers);
    ...
    $scope.move46 = function () {
		...
		for(var i=0;i<data2.length;i++){
			...
			object1.bom_level=parseInt(object1.pi_id);
			data1.push(object1);
			$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[rowidx].sale_pi_m_break_itemallofsale_pi_m_break_headers.push(object1);
		}
		for(var i=0;i<node.length;i++){
		...
		$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[rowidx].sale_pi_m_break_itemofsale_pi_m_break_headers=(data1);
		//$scope.data.currItem.sale_pi_m_break_headerofsale_pi_m_headers[rowidx].sale_pi_m_break_itemallofsale_pi_m_break_headers=(data1);
    ...
    $scope.columns_21 = [
		...
		{
            headerName: "预算销售价格", field: "pre_price", editable: false, filter: 'set', width: 150,
            cellEditor: "浮点框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
		...
		{
            headerName: "预算指导价格", field: "pre_guid_price", editable: false, filter: 'set', width: 150,
            cellEditor: "浮点框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
        ...
        {
            headerName: "选择该行时间", field: "price_time", editable: false, filter: 'set', width: 100,
            cellEditor: "年月日",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
		...
		{
            headerName: "上一次成交价格", field: "last_price", editable: false, filter: 'set', width: 150,
            cellEditor: "浮点框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
		...
		{
            headerName: "上次订单日期", field: "last_date", editable: false, filter: 'set', width: 150,
            cellEditor: "年月日",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
...
//包装方案（未维护）
	...
	$scope.exportjy3 = function () {
		if (!$scope.data.currItem.pi_m_id) {
			BasemanService.notice("未识别单据", "alert-info");
			return;
		}
		BasemanService.RequestPost("sale_pi_m_header", "exporttoexceljy3", {'pi_m_id': $scope.data.currItem.pi_m_id})
			.then(function (data) {
	...
	$scope.exportjy4 = function () {
		...
		BasemanService.RequestPost("sale_pi_m_header", "exporttoexceljy4", {'pi_m_id': $scope.data.currItem.pi_m_id})
			.then(function (data) {
				if (data.docid != undefined && Number(data.docid) != 0) {
					var fileurl = '/downloadfile.do?docid=' + data.docid + '&loginguid=' + window.strLoginGuid + '&t=' + new Date().getTime();
					window.open(fileurl, '_blank', 'height=100, width=400, top=0,left=0, toolbar=no,menubar=no, scrollbars=no, resizable=no,location=no, status=no');
...
//加载控制器
billmanControllers
    .controller('sale_pi_m_headerEdit', sale_pi_m_headerEdit)
