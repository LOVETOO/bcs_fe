...
function sale_zy_m_headerEdit($scope, $http, $rootScope, $modal, $location, $timeout, BasemanService, $state, localeStorageService, FormValidatorService, HistoryDataService, notify) {
    //继承基类方法
    sale_zy_m_headerEdit = HczyCommon.extend(sale_zy_m_headerEdit, ctrl_bill_public);
    sale_zy_m_headerEdit.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
        ...
        FrmInfo: {
            ...
            sqlBlock: "nvl(bill_flag,0)=2 and pi_m_no like 'ZY%'",
        },
        grids: [
            {
                optionname: 'h_itemoptions',
                idname: 'sale_pi_m_item_h_lineofsale_pi_m_headers',
                line: {optionname: "itemoptions", idname: "sale_pi_m_item_lineofsale_pi_m_item_h_lines"}
            },//订单明细  分体机
            {optionname: 'allpartoptions', idname: 'sale_pi_m_item_partofsale_pi_m_headers'},//配件清单
...

            {
                ...
                idname: 'sale_pi_m_item_lineofsale_pi_m_headers',
            }
        ]
    }
    ...
    $scope.selectpi_no =function(){
        ...
        $scope.FrmInfo = {
            ...
            sqlBlock:" stat=5 and pi_no like 'ZY%'"
        };
    ...
    $scope.set_line_conf = function () {
        var h_item = $scope.gridGetRow("h_itemoptions", "First");
        if (!h_item) {
            return;
        }
        var item_line_seq = h_item.seq;
        var item = $scope.gridGetRow("itemoptions", "First");
        if (!h_item) {
            return;
        }
        var item_id = item.item_id;
        var confs = $scope.gridGetData("options_32");
        var itemconfs = [];
        for (var i = 0; i < confs.length; i++) {
            if (confs[i].item_line_seq == item_line_seq && confs[i].item_id == item_id) {
                itemconfs.push(confs[i]);
            }
        }
        $scope.gridSetData("options_31", itemconfs);
    }
    ...
    $scope.save_before = function () {
        for (var i = 0; i < $scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers.length; i++) {
            $scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[i].seq = parseInt(i + 1);
        }
    ...
    BasemanService.RequestPostAjax("base_search", "searchdict", {dictcode: "line_type"}).then(function (data) {
        for (var i = 0; i < data.dicts.length; i++) {
            ...
            //订单明细
            $scope.h_itemcolumns[$scope.getIndexByField("h_itemcolumns", "line_type")].cellEditorParams.values.push(newobj);
        }
    })
    // 机型类别 pro_type
    BasemanService.RequestPostAjax("base_search", "searchdict", {dictcode: "pro_type"}).then(function (data) {
        for (var i = 0; i < data.dicts.length; i++) {
            ...
            //分体机
            $scope.itemcolumns[$scope.getIndexByField("itemcolumns", "pro_type")].cellEditorParams.values.push(newobj)
    ...
    BasemanService.RequestPostAjax("base_search", "searchdict", {dictcode: "sj_type"}).then(function (data) {
        for (var i = 0; i < data.dicts.length; i++) {
            ...
            //分体机
            $scope.itemcolumns[$scope.getIndexByField("itemcolumns", "sj_type")].cellEditorParams.values.push(newobj);
            //打散-包装方案
    ...
    $scope.area_names21 = function () {
        ...
        BasemanService.open(CommonPopController, $scope)
            .result.then(function (result) {
            var data = [];
            var index = $scope.h_itemoptions.api.getFocusedCell().rowIndex;
            var node = $scope.h_itemoptions.api.getModel().rootNode.childrenAfterSort;
            for (var i = 0; i < node.length; i++) {
                data.push(node[i].data)
            }
            ...
            $scope.h_itemoptions.api.setRowData(data);
    ...
    $scope.item_h_code21 = function () {
        ...
        $scope.FrmInfo.postdata.pro_type = $scope.h_itemoptions.api.getModel().rootNode.childrenAfterSort[$scope.h_itemoptions.api.getFocusedCell().rowIndex].data.line_type
    ...
    $scope.cust_changepi = function (postdata) {
        ...
        promise.then(function (data) {
            ...
            // 重选客户 -- 清空 产品明细和其细表
            $scope.h_itemoptions.api.setRowData([]);
    ...
    $scope.getxbrate = function () {
        ...
        var promise = BasemanService.RequestPost("sale_pi_m_header", "getxbrate", postdata);
        promise.then(function (data) {
    ...
    $scope.getpartlimit = function (cust_id) {
        ...
        var promise = BasemanService.RequestPost("sale_pi_m_header", "getpartlimit", postdata);
        promise.then(function (data) {
            data.part_limit;
        });
    }
    ...
    $scope.proItem = function (pro_type, index, item_h_id) {
        ...
        promise.then(function (data) {
            ...
            $scope.itemoptions.api.setRowData($scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[index].sale_pi_m_item_lineofsale_pi_m_item_h_lines);
            var l = {};
            l.sale_pi_m_item_h_lineofsale_pi_m_headers = [];
            l.sale_pi_m_item_h_lineofsale_pi_m_headers.push($scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[index]);
            var postdata = l;
    ...
    $scope.selectprice = function (postdata, line_type, dataheader, index) {
        if (line_type != 4) {
            ...
            promise.then(function (data) {
                ...
                $scope.h_itemoptions.api.setRowData($scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers);
                $scope.itemoptions.api.setRowData(dataline);
    ...
    $scope.computeqty = function (qty) {
        var val = qty;
        var index = $scope.h_itemoptions.api.getFocusedCell().rowIndex;
        var nodes = $scope.h_itemoptions.api.getModel().rootNode.childrenAfterSort;
        var itemtemp = nodes[index].data;
        var pi_m_item_h_lines = itemtemp.sale_pi_m_item_lineofsale_pi_m_item_h_lines;
        var qty_temp = 0;
        var sum_qty = 0;
        var sum_amt = 0;
        //itemtemp.price = itemtemp.price ? itemtemp.price : 0;
        //itemtemp.sale_amt = parseFloat(val || 0) * parseFloat(itemtemp.price);
        // $scope.h_itemoptions.api.refreshCells(nodes, ["sale_amt"]);

        if (index != undefined) {
            val = val ? val : 0;
            for (var i = 0; i < pi_m_item_h_lines.length; i++) {
                if (pi_m_item_h_lines.length > 0) {
                    pi_m_item_h_lines[i].qty = parseInt(val);
                }
                ;
            }
            ;
            if (pi_m_item_h_lines.length == 1) {
                pi_m_item_h_lines[0].qty = parseInt(val || 0);
                pi_m_item_h_lines[0].price = parseFloat(itemtemp.price || 0)
                pi_m_item_h_lines[0].sale_amt = parseFloat(pi_m_item_h_lines[0].price || 0) * parseInt(pi_m_item_h_lines[0].qty || 0);
            } else {
                var sale_amt1 = 0;
                var price1 = 0;
                for (var i = 0; i < pi_m_item_h_lines.length; i++) {
                    if (parseInt(itemtemp.guide_p6 || 0) == 0) {
                        ...
                        pi_m_item_h_lines[i].price = HczyCommon.toDecimal2(Number((HczyCommon.toDecimal2(pi_m_item_h_lines[i].guide_p6 || 0) / HczyCommon.toDecimal2(itemtemp.guide_p6 || 0) * HczyCommon.toDecimal2(itemtemp.price || 0) * 100) / 100));
                        pi_m_item_h_lines[i].sale_amt = HczyCommon.toDecimal2(pi_m_item_h_lines[i].price * parseInt(pi_m_item_h_lines[i].qty || 0));
                    }

                }
                if (pi_m_item_h_lines.length - 1 > 0) {
                    for (var j = 0; j < pi_m_item_h_lines.length - 1; j++) {
                        price1 = HczyCommon.toDecimal2(parseFloat(price1 || 0) + parseFloat(pi_m_item_h_lines[j].price || 0));
                        sale_amt1 = HczyCommon.toDecimal2(parseFloat(sale_amt1 || 0) + parseFloat(pi_m_item_h_lines[j].sale_amt || 0))
                    }
                    price1 = parseFloat(HczyCommon.toDecimal2(price1));
                    sale_amt1 = parseFloat(HczyCommon.toDecimal2(sale_amt1));
                    pi_m_item_h_lines[pi_m_item_h_lines.length - 1].sale_amt = HczyCommon.toDecimal2(parseFloat(itemtemp.sale_amt || 0) - sale_amt1);
                    pi_m_item_h_lines[pi_m_item_h_lines.length - 1].price = HczyCommon.toDecimal2(parseFloat(itemtemp.price || 0) - price1);
                }
            }
            $scope.itemoptions.api.setRowData(pi_m_item_h_lines);
        }
    };
    //判断是否有易损件
    ...
    $scope.getoneYS = function (sale_amt, items) {
        ...
        if (items[0].sale_pi_m_item_h_lineofsale_pi_m_headers.length) {

            object = items[0].sale_pi_m_item_h_lineofsale_pi_m_headers[0];
    ...
    /**网格切换事件*/
    //订单明细  关联分体机和单前机型
    $scope.defaultcoldef = $.extend(true, {}, ($scope.h_itemcolumns));
    $scope.rowClicked21 = function (event) {
        //控制网格单行只读
        var colmun=$scope.h_itemoptions.columnApi.getAllColumns();
        var temp=$scope.h_itemoptions.api.getModel().rootNode.childrenAfterSort[$scope.h_itemoptions.api.getFocusedCell().rowIndex].data;
        if(temp.line_type==4){
            ...
            var colmun=$scope.itemoptions.columnApi.getAllColumns();
        ...
        $scope.itemoptions.api.setFocusedCell(-1,"cust_item_name");
        //分体机==
        if (event.data) {
            ...
            if($scope.itemoptions.api.getFocusedCell()){
                if($scope.itemoptions.api.getFocusedCell().rowIndex!=$scope.h_itemoptions.api.getFocusedCell().rowIndex){
                    $scope.itemoptions.api.setRowData(event.data.sale_pi_m_item_lineofsale_pi_m_item_h_lines);
                }
            }else{
                $scope.itemoptions.api.setRowData(event.data.sale_pi_m_item_lineofsale_pi_m_item_h_lines);
            }
        ...
        //单前机型
        if ($scope.itemoptions.api.getModel().rootNode.childrenAfterSort) {
            var node = $scope.itemoptions.api.getModel().rootNode.childrenAfterSort;
            var postdata = {};
    ...
    $scope.rowClicked22 = function () {
        var rowidx = $scope.itemoptions.api.getFocusedCell().rowIndex;
        if ($scope.itemoptions.api.getModel().rootNode.childrenAfterSort) {
            var node = $scope.itemoptions.api.getModel().rootNode.childrenAfterSort;
            var postdata = {};
    ...
    $scope.qty21 = function () {
        ...
        //var node1s=$scope.itemoptions.api.getModel().rootNode.childrenAfterSort;
        ...
        if (index != undefined) {
            ...
            for (var i = 0; i < pi_m_item_h_lines.length; i++) {
                if (pi_m_item_h_lines.length > 0) {
                    pi_m_item_h_lines[i].qty = Number(val);
                }
                ;
            }
            ;
            if (pi_m_item_h_lines.length == 1) {
                pi_m_item_h_lines[0].qty = Number(val);
                pi_m_item_h_lines[0].sale_amt = Number(pi_m_item_h_lines[0].price || 0) * parseInt(pi_m_item_h_lines[0].qty || 0);
            } else {
                var sale_amt1 = 0;
                var price1 = 0;
                for (var i = 0; i < pi_m_item_h_lines.length; i++) {
                    if (parseInt(itemtemp.guide_p6 || 0) == 0) {
                        pi_m_item_h_lines[i].price = 0;
                        pi_m_item_h_lines[i].sale_amt = 0
                    } else {
                        pi_m_item_h_lines[i].price = HczyCommon.toDecimal2(Number((HczyCommon.toDecimal2(pi_m_item_h_lines[i].guide_p6 || 0) / HczyCommon.toDecimal2(itemtemp.guide_p6 || 0) * HczyCommon.toDecimal2(itemtemp.price || 0) * 100) / 100));
                        pi_m_item_h_lines[i].sale_amt = HczyCommon.toDecimal2(pi_m_item_h_lines[i].price * parseInt(pi_m_item_h_lines[i].qty || 0));
                    }
    ...
    $scope.price21 = function () {
        ...
        if (index != undefined) {
            val = val ? val : 0;
            if (pi_m_item_h_lines.length == 1) {
                pi_m_item_h_lines[0].price = Number(val || 0);
                pi_m_item_h_lines[0].sale_amt = parseFloat(pi_m_item_h_lines[0].price || 0) * parseInt(pi_m_item_h_lines[0].qty || 0);
                pi_m_item_h_lines[0].sale_amt = HczyCommon.toDecimal2(pi_m_item_h_lines[0].sale_amt);
            } else {
                var price_1 = 0;
                var sale_amt1 = 0;
                var price1 = 0;
                for (var i = 0; i < pi_m_item_h_lines.length; i++) {
                    pi_m_item_h_lines[i].qty = pi_m_item_h_lines[i].qty ? pi_m_item_h_lines[i].qty : 0;
                    if (parseInt(itemtemp.guide_p6) == 0) {
                        pi_m_item_h_lines[i].price = 0;
                        pi_m_item_h_lines[i].sale_amt = 0;
                    } else {
                        pi_m_item_h_lines[i].price = HczyCommon.toDecimal2((HczyCommon.toDecimal2(pi_m_item_h_lines[i].guide_p6 || 0) / HczyCommon.toDecimal2(itemtemp.guide_p6 || 0)) * HczyCommon.toDecimal2(val || 0));
                        pi_m_item_h_lines[i].sale_amt = HczyCommon.toDecimal2(pi_m_item_h_lines[i].price * parseInt(pi_m_item_h_lines[i].qty || 0));
                        price_1 = HczyCommon.toDecimal2(parseFloat(price_1) + parseFloat(pi_m_item_h_lines[i].price || 0));
                    }
                }
                if (pi_m_item_h_lines.length - 1 > 0) {
                    for (var j = 0; j < pi_m_item_h_lines.length - 1; j++) {
                        price1 = (parseFloat(price1 || 0) + parseFloat(pi_m_item_h_lines[j].price || 0));
                        sale_amt1 = (parseFloat(sale_amt1 || 0) + parseFloat(pi_m_item_h_lines[j].sale_amt || 0));
                    }
                    price1 = parseFloat(HczyCommon.toDecimal2(price1));
                    sale_amt1 = parseFloat(HczyCommon.toDecimal2(sale_amt1));
                    pi_m_item_h_lines[pi_m_item_h_lines.length - 1].sale_amt = HczyCommon.toDecimal2(parseFloat(itemtemp.sale_amt || 0) - parseFloat(sale_amt1 || 0));
                    pi_m_item_h_lines[pi_m_item_h_lines.length - 1].price = HczyCommon.toDecimal2(parseFloat(val || 0) - parseFloat(price1 || 0));
                }
            }
            $scope.itemoptions.api.setRowData(pi_m_item_h_lines);
        }
    ...
    $scope.require_qty23 = function () {
        var _this = $(this);
        var val = _this.val();
        var index = $scope.allpartoptions.api.getFocusedCell().rowIndex;
        var nodes = $scope.allpartoptions.api.getModel().rootNode.childrenAfterSort;
        nodes[index].data.amt = parseFloat(nodes[index].data.psale_price || 0) * parseInt(val);
        $scope.allpartoptions.api.refreshCells(nodes, ["amt"]);
    }
    ...
    $scope.generate16 = function () {
        var postdata = {};
        postdata.pi_m_id = parseInt($scope.data.currItem.pi_m_id);
        BasemanService.RequestPost("sale_pi_m_header", "creatembbill", postdata)
            .then(function (data) {
                $scope.refresh(2);
    ...
    $scope.addline21 = function () {
        var brand_id = "";
        //var test=$scope.h_itemoptions.rowData;
        var data = [];
        var node = $scope.h_itemoptions.api.getModel().rootNode.childrenAfterSort;
        for (var i = 0; i < node.length; i++) {
            data.push(node[i].data);
    ...
    $scope.delline21 = function () {
        var data = [];
        var rowidx = $scope.h_itemoptions.api.getFocusedCell().rowIndex;
        var node = $scope.h_itemoptions.api.getModel().rootNode.childrenAfterSort;
        for (var i = 0; i < node.length; i++) {
            data.push(node[i].data);
        }
    ...
    //订单明细 订单选配
    $scope.open = function () {
        if ($scope.data.currItem.pi_m_id == undefined || $scope.data.currItem.pi_m_id == 0) {
        ...
        if ($scope.data.currItem.stat > 1) {
            var pcsurl = "http://100.100.68.5:8081/ocm/login.jsp?user_id=538&randompass=AVMNOGAKKEIPSLUI&qo_id=" + $scope.data.currItem.pi_m_id + "&readonly=1&fullscreen=1&oms_org_id=1500&is_new=1&bill_type=40038";
        } else {
            var pcsurl = "http://100.100.68.5:8081/ocm/login.jsp?user_id=538&randompass=AVMNOGAKKEIPSLUI&qo_id=" + $scope.data.currItem.pi_m_id + "&readonly=0&fullscreen=1&oms_org_id=1500&is_new=1&bill_type=40038";
        }
    ...
    $scope.dataup = function () {
        var cell = $scope.h_itemoptions.api.getFocusedCell();
        var rowidx = $scope.h_itemoptions.api.getFocusedCell().rowIndex;
        var nodes = $scope.h_itemoptions.api.getModel().rootNode.childrenAfterSort;
        var data = [];
        if (rowidx == 0) {
        ...
        $scope.h_itemoptions.api.setFocusedCell(rowidx - 1, cell.column.colId);
        $scope.h_itemoptions.api.setRowData(data);
    }
    ...
    $scope.h_itemcolumns = [
        ...
        {
            headerName: "选择改行时间", field: "price_time", editable: false, filter: 'set', width: 100,
            cellEditor: "年月日",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
    ...
    $scope.allpartoptions = {
        ...
        showToolPanel: false,
        checkboxSelection: function (params) {
            var isGrouping = $scope.allpartoptions.columnApi.getRowGroupColumns().length > 0;
            return params.colIndex === 0 && !isGrouping;
        },
        icons: {
            columnRemoveFromGroup: '<i class="fa fa-remove"/>',
    ...
    $scope.save_before = function () {
        ...
        delete $scope.data.currItem.sale_pi_m_break_itemofsale_pi_m_headers
        delete $scope.data.currItem.sale_pi_item_lineofsale_pi_headers
        ...
        delete $scope.data.currItem.sale_pi_m_item_lineofsale_pi_m_header;
    }
    ...
    $scope.refresh_after = function () {
        ...
        var datas = $scope.data.currItem.sale_pi_m_item_lineofsale_pi_m_headers;
        var obj = {};
        for (var i = 0; i < $scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers.length; i++) {
            obj = $scope.data.currItem.sale_pi_m_item_h_lineofsale_pi_m_headers[i];
            datas = datas.concat(obj.sale_pi_m_item_lineofsale_pi_m_item_h_lines);
        }
...
//加载控制器
billmanControllers
    .controller('sale_zy_m_headerEdit', sale_zy_m_headerEdit)
