...
function sale_ship_warn_headerXGEdit($scope, $location, $http, $rootScope, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService, HistoryDataService) {

    //继承基类方法
    sale_ship_warn_headerXGEdit = HczyCommon.extend(sale_ship_warn_headerXGEdit, ctrl_bill_public);
    sale_ship_warn_headerXGEdit.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
    ...
    $scope.headername="出货预告编辑"

    $scope.ExpandValue = 0;
    if ($scope.$state.current.url =="/sale_ship_warn_headerCQEdit"||$scope.$state.current.name == "crmman.sale_ship_warn_headerCQEdit") {//船期修改
        $scope.ExpandValue = 10;
        $scope.objconf.FrmInfo.sqlBlock=" Stat = 5 ";
        $("ul#tab1tab li:first").removeClass("active");
        $("ul#tab1tab li:nth-child(3)").addClass("active");
        $("#tab1a").removeClass("active");
        $("#tab1c").addClass("tab-pane fade in active");
        $scope.headername="出货预告船期更改";
    } else if ($scope.$state.current.url =="/sale_ship_warn_headerOPEdit"||$scope.$state.current.name == "crmman.sale_ship_warn_headerOPEdit") {//出货港修改
        $scope.headername="出货预告出货港更改";
        $scope.ExpandValue = 9;
        $scope.objconf.FrmInfo.sqlBlock="Stat >= 1 and nvl(warn_type,0) !=2";
    } else if ($scope.$state.current.url =="/sale_ship_warn_headerFJEdit"||$scope.$state.current.name == "crmman.sale_ship_warn_headerFJEdit") {//附件修改
        $scope.ExpandValue = 20;
        $scope.headername="出货预告追加附件";
        $scope.objconf.FrmInfo.sqlBlock="Stat >= 1 and nvl(warn_type,0) !=2";
    }

    $scope.save_before = function () {
        if ($scope.ExpandValue == 10) {
            $scope.data.currItem.flag = 110;
        } else if ($scope.ExpandValue == 9) {
            $scope.data.currItem.flag = 9;
        } else if ($scope.ExpandValue == 20) {
    ...
    /**网格配置*/
    {
        ...
        //cntoptions
        {
            ...
            $scope.sale_ship_warn_lc_lineFrm = function () {
                var pi_id = "-1";
                //var lc_bill_id="-1";
                var data = $scope.aoptions.grid.getData();
                if (data.length == 0) {
                    BasemanService.notice("增加产品明细在选择信用证!");
                    return;
                }
                for (i = 0; i < data.length; i++) {
                    pi_id = data[i].pi_id + "," + pi_id;
                    //lc_bill_id=data[i].lc_bill_id + "," + lc_bill_id;
                }
                $scope.FrmInfo = {
                    sqlBlock: " 1=1 and 10000=10000 " + " and pi_id in (" + pi_id + ")",
                    classid: "fin_lc_bill",
                }
                $scope.FrmInfo.postdata = {};
                $scope.FrmInfo.postdata.flag = 3;
                BasemanService.open(CommonPopController, $scope, "", "lg").result.then(function (result) {
                    var data = $scope.cntoptions.grid.getData();
                    for (i = 0; i < data.length; i++) {
                        if (data[i].lc_bill_id == result.lc_bill_id) {
                            BasemanService.notice("该信用证已经被选择,请不要重复添加!", "alert-warning");
                            return;
                        }
                    }
                    // 前台界面中的两列的显示关系调整了一下
                    result.lc_bill_no = result.lc_no;
                    result.last_Tran_Date = result.latest_shipment_date;
                    $scope.cntoptions.grid.getData().push(result);
...
//加载控制器
billmanControllers
    .controller('sale_ship_warn_headerXGEdit', sale_ship_warn_headerXGEdit)
