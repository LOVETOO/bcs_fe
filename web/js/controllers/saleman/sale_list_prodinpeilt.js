...
function sale_list_prodinpeilt($scope, $location, $rootScope, $modal, notify, $timeout, BasemanService, $state, localeStorageService, FormValidatorService) {
    //继承基类方法
    sale_list_prodinpeilt = HczyCommon.extend(sale_list_prodinpeilt, ctrl_bill_public);
    sale_list_prodinpeilt.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
        name: "edi_plnotice_list",
   /*     key: "funds_id",*/
        wftempid: 10008,
        FrmInfo: {
            sqlBlock: "funds_type=2",
        },
        grids: [{optionname: 'options_11', idname: 'edi_plnotice_lists'}/*, {
   ...
   $scope.search = function () {
	...
	var postdata = { //flag:8,
		   ...
		   area_code:  $scope.data.currItem.org_codedq,
           area_name: $scope.data.currItem.org_namedq,
			cust_id: $scope.data.currItem.cust_codea,
		  cust_code: $scope.data.currItem.cust_codea,
 ...
 BasemanService.RequestPost("edi_plnotice_list", "search", postdata)
        .then(function(data){
            $scope.data.currItem.edi_plnotice_lists=data.edi_plnotice_lists;
             $scope.options_11.api.setRowData(data.edi_plnotice_lists);
        });
}
    //业务部门
   ...
   $scope.selectcust = function () {
    ...
    $scope.groupby = function (arr,column,datas) {
        ...
        if ($scope.data.currItem.check3 == true) {
            arr.push("corpserialno");
            $scope.options_12.columnApi.hideColumns(["corpserialno"])
            sortmodel.push({colId:"corpserialno",sort:"desc"})
        }
        if ($scope.data.currItem.check4 == true) {
            arr.push("cust_name");
            $scope.options_12.columnApi.hideColumns(["cust_name"])
    ...
    $scope.columns_11 = [
        {
            headerName: "可损通知号", field: "corpserialno", editable: false, filter: 'set', width: 100,
            cellEditor: "下拉框",
/*            cellEditorParams: {
            ...
            headerName: "发票金额", field: "invoicesum", editable: false, filter: 'set', width: 150,
            cellEditor: "文本框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
            ...
            headerName: "报损金额", field: "plsum", editable: false, filter: 'set', width: 100,
            cellEditor: "文本框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
            ...
            headerName: "收款", field: "receiptsum", editable: false, filter: 'set', width: 100,
            cellEditor: "文本框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
            ...
            headerName: "最终索赔金额", field: "last_amt", editable: false, filter: 'set', width: 150,
            cellEditor: "文本框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
            ...
            headerName: "坏账处理金额", field: "bad_amt", editable: false, filter: 'set', width: 150,
            cellEditor: "文本框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
            ...
            headerName: "赔付比例", field: "prate", editable: false, filter: 'set', width: 100,
            cellEditor: "文本框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
...
//加载控制器
finmanControllers
    .controller('sale_list_prodinpeilt', sale_list_prodinpeilt);
