...
function sale_std_priceapply_headerEdit($scope, $location, $http, $rootScope, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService, HistoryDataService) {

    //继承基类方法
    sale_std_priceapply_headerEdit = HczyCommon.extend(sale_std_priceapply_headerEdit, ctrl_bill_public);
    sale_std_priceapply_headerEdit.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
        ...
        grids: [
            {
                ...
                idname: 'sale_std_priceapply_line2ofsale_std_priceapply_headers'
            }
        ]
    };
    /**网格配置*/
                ...
                BasemanService.open(CommonPopController, $scope)
                    ...
                    for(var i=0;i<datas.length;i++){
                        ...
                        if(notexists){
                            ...
                            if(obj.pass){
                                $scope.data.currItem.sale_std_priceapply_line2ofsale_std_priceapply_headers=
                                    $scope.data.currItem.sale_std_priceapply_line2ofsale_std_priceapply_headers.concat(obj.data.pro_items);
                            }
                        }
                    }
            ...
            $scope.del_line=function () {
                ...
                for(var i=0;i<$scope.data.currItem.sale_std_priceapply_line2ofsale_std_priceapply_headers.length;i++){
                    ...
                    if(obj.item_h_id==data.item_h_id){
                        $scope.data.currItem.sale_std_priceapply_line2ofsale_std_priceapply_headers.splice(i,1);
                    }
                }
...
billmanControllers
    .controller('sale_std_priceapply_headerEdit', sale_std_priceapply_headerEdit)
