            ...
            function ($scope, $stateParams, BasemanService, $q) {
                ...
                $scope.gridOptions = {
                    ...
                    columnDefs: [
                        ...
                        , {
                            field: 'qty_onhand',
                            headerName: '接收仓实际库存数量',
                            type: '数量'
                        }, {
                            field: 'qty_plan',
                            headerName: '接收可使用库存数量',
                            type: '数量'
                        },
                        {
                            field: 'qty_bill',
                ...
                $scope.add_line = function () {
                    ...
                    swal({
                        ...
                        var data = $scope.data.currItem.sa_out_bill_lines;
                ...
                $scope.del_line = function () {
                    ...
                    return swalApi.confirmThenSuccess({
                        ...
                        okFun: function () {
                            ...
                            $scope.data.currItem.sa_out_bill_lines = rowData;
                        },
                        okTitle: '删除成功'
                    });
                }
                ...
                $scope.validCheck = function (invalidBox) {
                    ...
                    $.each($scope.data.currItem.sa_out_bill_lines, function (i, row) {
                        var attribute41 = row.attribute41;//代销仓可使用库存数量

                        //参考价不能为0（除非赠品选择项打勾）
                        if (!row.attribute11 || (parseInt(row.attribute11) < 2 && (HczyCommon.isNull(row.reference_price) || parseFloat(row.reference_price) <= 0))) {
                            invalidBox.push('第' + (i + 1) + '行参考价不能为0');
                        }
                        if (!row.qty_bill) {
                            invalidBox.push('第' + (i + 1) + '行退货数量不能为空');
                        }
                        else if (numberApi.compare(attribute41, row.qty_bill) == -1) {
                            invalidBox.push('第' + (i + 1) + '行代销仓[' + $scope.data.currItem.proxy_warehouse_name + ']' +
                                '产品[' + row.item_name + ']可使用库存数量[' + attribute41 + ']小于退货数量[' + row.qty_bill + ']');
                        }
                    })
                ...
                function countCubage(data) {
                    if (HczyCommon.isNotNull(data.qty_bill) && HczyCommon.isNotNull(data.cubage)) {
                        data.goods_cubage = numberApi.mutiply(data.qty_bill, data.cubage);
                    }
                    countTotal_Cubage();
                }
                ...
                function countAmount_bill(data) {
                    if (HczyCommon.isNotNull(data.qty_bill) && HczyCommon.isNotNull(data.reference_price)) {
                ...
                function countTotal_Amount_bill() {
                    var amount_total = numberApi.sum($scope.data.currItem.sa_out_bill_lines, 'amount_bill');
                ...
                function countQty_sum() {
                    var qty_sum = numberApi.sum($scope.data.currItem.sa_out_bill_lines, 'qty_bill');
                ...
                function countTotal_Cubage() {
                    var total_cubage = numberApi.sum($scope.data.currItem.sa_out_bill_lines, 'goods_cubage');
                    ...
                    $scope.gridOptions.api.setPinnedBottomRowData([
                        {
                            ...
                            goods_cubage: numberApi.sum($scope.data.currItem.sa_out_bill_lines, 'goods_cubage'),
                        }
                    ]);
                }
                ...
                $scope.newBizData = function (bizData) {
                    ...
                    $scope.gridOptions.api.setRowData($scope.data.currItem.sa_out_bill_lines);
                };
                ...
                $scope.chooseUser = function () {
                    BasemanService.chooseUser({
                        ...
                        then: function (response) {
                            $scope.data.currItem.created_by = response.employee_name;
                        }
                    })
                };
                ...
                function chooseWareHouse(args) {
                    ...
                    if (args && args == 'sa') {
                        sqlBlock = ' usable =2 and warehouse_type = 1 and warehouse_property = 4 and is_end = 2 ';//搜实物代销仓\明细仓
                    ...
                    BasemanService.open(CommonPopController, $scope).result.then(function (response) {
                        if (args && args.data) {
                            ...
                            $scope.data.currItem.sa_out_bill_lines.forEach(function (row) {
                                getDaixiaoStock(row);
                            })
                        } else {
                 ...
                 * 查客户地址
                 */
                $scope.commonSearchSettingOfAddress = {
                    beforeOpen: function(){
                ...
                $scope.commonSearchSettingOfCustomerOrg = {
                    sqlWhere: 'co.usable = 2 and co.sale_type = 5 ',
                    afterOk: function (result) {
                ...
                //         title: '选择客户',
                //         scope: $scope,
                //         sqlBlock: 'co.usable = 2 and co.sale_type = 5 ',
                ...
                function chooseItem(args) {
                    ...
                    return BasemanService
                        ...
                        .then(function (data) {
                            ...
                            args.data.cubage = data.cubage;
                            return args.data;
                        })
                        .then(getSpecialPrice)//取特价
                        .then(getDaixiaoStock)//查代销库存
                        .then(getNormalStock) //查接受仓库存
                        .then(countCubage)
                        .then(function () {
                            args.api.refreshView();
                        })
                    ...
                    BasemanService.open(CommonPopController, $scope).result
                        ...
                        .then(function (response) {
                            loopApi.forLoop(response.sa_out_bill_lines.length, function (i) {
                                response.sa_out_bill_lines[i].p_inv_out_bill_line_id
                                    = response.sa_out_bill_lines[i].inv_out_bill_line_id;

                                response.sa_out_bill_lines[i].invbillno = $scope.data.currItem.parent_billno;
                            });
                            $scope.gridOptions.hcApi.setRowData(response.sa_out_bill_lines);
                            $scope.data.currItem.sa_out_bill_lines = response.sa_out_bill_lines;
                            calSum();
                        })
                };
