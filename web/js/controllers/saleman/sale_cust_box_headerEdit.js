...
function sale_cust_box_headerEdit($scope, $location, $rootScope, $modal, notify, $timeout, BasemanService, $state, localeStorageService, FormValidatorService) {
    //继承基类方法
    sale_cust_box_headerEdit = HczyCommon.extend(sale_cust_box_headerEdit, ctrl_bill_public);
    sale_cust_box_headerEdit.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
        name: "sale_cust_box_header",
        key: "cust_box_id",
        wftempid: 10131,
        FrmInfo: {},
        grids: [{optionname: "options1", idname: "sale_cust_box_lineofsale_cust_box_headers"}]
    };
    /**----弹出框区域*---------------*/
    //界面初始化
    //分组功能
    ...
    $scope.refresh_after = function () {
        $scope.gridSetData("options1", $scope.data.currItem.sale_cust_box_lineofsale_cust_box_headers);
    }
...
//加载控制器
basemanControllers
    .controller('sale_cust_box_headerEdit', sale_cust_box_headerEdit);
