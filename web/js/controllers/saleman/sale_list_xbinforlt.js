...
function sale_list_xbinforlt($scope, $location, $rootScope, $modal, notify, $timeout, BasemanService, $state, localeStorageService, FormValidatorService) {
    //继承基类方法
    sale_list_xbinforlt = HczyCommon.extend(sale_list_xbinforlt, ctrl_bill_public);
    sale_list_xbinforlt.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
        name: "edi_insurance_list",
   /*     key: "funds_id",*/
        wftempid: 10008,
        FrmInfo: {
            sqlBlock: "funds_type=2",
        },
        grids: [{optionname: 'options_11', idname: 'edi_insurance_lists'}/*, {
 ...
 BasemanService.RequestPost("edi_insurance_list", "search", postdata)
        .then(function(data){
            $scope.data.currItem.edi_insurance_lists=data.edi_insurance_lists;
             $scope.options_11.api.setRowData(data.edi_insurance_lists);
        });
}
...
/*            cellEditorParams: {
            ...
            headerName: "出运申报单号", field: "corpserialno", editable: false, filter: 'set', width: 150,
            cellEditor: "文本框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
            ...
            headerName: "出运申报上信保比率", field: "ship_rate", editable: false, filter: 'set', width: 250,
            cellEditor: "文本框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
            ...
            headerName: "信保费用", field: "insurefee", editable: false, filter: 'set', width: 100,
            cellEditor: "文本框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
...
//加载控制器
finmanControllers
    .controller('sale_list_xbinforlt', sale_list_xbinforlt);
