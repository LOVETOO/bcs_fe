...
function sale_prod_header($scope, $location, $rootScope, $modal, $timeout, BasemanService, notify, $state, localeStorageService, FormValidatorService){
    //设置当前对象名及主键名
    $scope.objconf = {
        classid: "sale_prod_header",
        key: "prod_id",
        nextStat: "sale_prod_headerEdit",
    ...
    $scope.headername="生产单";
    sale_prod_header = HczyCommon.extend(sale_prod_header,ctrl_view_public);
    sale_prod_header.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal,
...
//			$scope.viewColumns[4].cellEditorParams.values.push(object);
    ...
    $scope.viewColumns = [	
		...
		{
             headerName: "是否承诺15天回定金", field: "is_cndk",editable: false, filter: 'set',  width: 150,
             cellEditor:"复选框",
             enableRowGroup: true,
             enablePivot: true,
             enableValue: true,
...
//加载控制器
basemanControllers
    .controller('sale_prod_header',sale_prod_header);
