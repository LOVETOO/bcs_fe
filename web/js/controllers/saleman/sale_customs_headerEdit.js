...
function sale_customs_headerEdit($scope, $location, $http, $rootScope, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService, HistoryDataService) {

    //继承基类方法
    sale_customs_headerEdit = HczyCommon.extend(sale_customs_headerEdit, ctrl_bill_public);
    sale_customs_headerEdit.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
        ...
        wftempid: 10079,
        FrmInfo: {
            qlBlock: " bill_type=2 or bill_type=1",
        },
        grids: [
            {//船务信息
        ...
        //cntoptions
        {
            ...
            $scope.addcntitem = function () {
                ...
                if ($scope.data.currItem.notice_id != undefined) {
                    $scope.FrmInfo.postdata.notice_man = $scope.data.currItem.notice_man || "";
                }
            ...
            $scope.cntcellchange = function () {
                    ...
                    if (price_h > 0) {
                        ...
                        for (var i = 0; i < lines.length; i++) {
                            if (i + 1 == lines.length) {
                                ...
                                lines[i].customs_price = (Number(price_1 || 0) / Number(h_detail.customs_price)).toFixed(4);
                                lines[i].customs_amt = (Number(lines[i].customs_price || 0) * Number(lines[i].customs_qty || 0)).toFixed(4);
                ...
                key = ["cusmtoms_amt", "customs_qty", "unit_nw", "unit_gw", "unit_tj","out_qty"]
    ...
    /*选择框*/
    {
        $scope.selectwarn_no = function () {
            ...
            if ($rootScope.$state.$current.self.name == "crmman.sale_customs_headerEdit") {
                $scope.FrmInfo.sqlBlock = "stat= 5";//正式报关单
            }
            BasemanService.open(CommonPopController, $scope, "", "lg")
        ...
        $scope.selectnotice = function () {
            ...
            $scope.FrmInfo = {
                ...
                type: 'checkbox',
                sqlBlock: " stat = 5 and nvl(is_customs,0)<>2  and warn_id =" + $scope.data.currItem.warn_id,
            };
            ...
            BasemanService.open(CommonPopController, $scope, "", "lg")
                ...
                BasemanService.RequestPost("sale_ship_notice_header", "baoguangzj", postdata).then(function (data) {
        ...
        $scope.back_print = function () {
            ...
            for (var i = 0; i < h_detail.length; i++) {
                ...
                for (var j = 0; j < lines.length; j++) {
                    if (lines[j].pro_type != "1" && lines[j].pro_type != "2"
                        && lines[j].pro_type != "3" && lines[j].line_type != "1") {
                        continue;
                    }
                    lines[j].print_price = lines[j].customs_price;
    ...
    $scope.to_formal = function (e) {
        if ($scope.data.currItem.bill_type == 2 && $scope.data.currItem.stat == 5) {
            try {
    ...
    }
    {
        ...
        $scope.exportjy1 = function () {
            ...
            BasemanService.RequestPost("sale_customs_header", "exporttoexceljy1", {'customs_id': $scope.data.currItem.customs_id})
                .then(function (data) {
        ...
        $scope.exportjy2 = function () {
            ...
            BasemanService.RequestPost("sale_customs_header", "exporttoexceljy2", {'customs_id': $scope.data.currItem.customs_id})
                .then(function (data) {
                    if (data.docid != undefined && Number(data.docid) != 0) {
                        var fileurl = '/downloadfile.do?docid=' + data.docid + '&loginguid=' + window.strLoginGuid + '&t=' + new Date().getTime();
                        window.open(fileurl, '_blank', 'height=100, width=400, top=0,left=0, toolbar=no,menubar=no, scrollbars=no, resizable=no,location=no, status=no');
...
billmanControllers
    .controller('sale_customs_headerEdit', sale_customs_headerEdit)
