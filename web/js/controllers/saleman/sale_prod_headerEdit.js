...
function sale_prod_headerEdit($scope, $location, $rootScope, $modal, notify, $timeout, BasemanService, $state, localeStorageService, FormValidatorService) {
    //继承基类方法
    sale_prod_headerEdit = HczyCommon.extend(sale_prod_headerEdit, ctrl_bill_public);
    sale_prod_headerEdit.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
	...
	// 类型 stress_note
    BasemanService.RequestPostAjax("base_search", "searchdict", {dictcode: "stress_type"}).then(function (data) {
        $scope.stress_types=data.dicts;
    })
    $scope.tab2 =function(){
		var postdata={};
	...
	$scope.summary =function(){
		var sum1=0,sum2=0;
		var row=$scope.data.currItem.sale_prod_break_headerofsale_prod_headers;
		if($scope.options_41.api.getFocusedCell()){
			...
			for(var i=0;i<row.length;i++){
				for(var j=0;j<row[i].sale_bill_break_itemofsale_prod_break_headers.length;j++){
					if(i==index){
					sum1+=parseFloat(row[i].sale_bill_break_itemofsale_prod_break_headers[j].package_qty||0)*parseFloat(row[i].sale_bill_break_itemofsale_prod_break_headers[j].package_tj||0)	
					}
					sum2+=parseFloat(row[i].sale_bill_break_itemofsale_prod_break_headers[j].package_qty||0)*parseFloat(row[i].sale_bill_break_itemofsale_prod_break_headers[j].package_tj||0)
				}
			}
    ...
    $scope.refresh_after = function () {
        ...
        for (var i = 0; i < $scope.data.currItem.sale_printed_lineofsale_prod_headers.length; i++) {
            data = data.concat($scope.data.currItem.sale_printed_lineofsale_prod_headers[i].sale_printed_lineofsale_prod_lines);
        }
        ...
        if (mystring.indexOf("计划管理") > -1) {
            for (var i = 0; i < $scope.columns_21.length; i++) {
                if ($scope.columns_21[i].field == "prod_time"||$scope.columns_21[i].field == "deliver_date") {
                    $scope.columns_21[i].editable = true;
                }
            }
			for (var i = 0; i < $scope.columns_22.length; i++) {
                if ($scope.columns_22[i].field == "prod_time"||$scope.columns_22[i].field == "deliver_date") {
    ...
    /*****************网格处理事件****************************/
    {
        ...
        $scope.PI_Split = function () {
            ...
            BasemanService.RequestPost("sale_prod_header", "splitprod", postdata)
                .then(function (data) {
        ...
        $scope.rowClicked41 = function (event) {
            ...
            if (event.data) {
                ...
                if (event.data.sale_prod_break_itemallofsale_prod_break_headers) {
					...
					$scope.deal_dataSelected("options_41","options_43",event.data.sale_prod_break_itemallofsale_prod_break_headers)
                ...
                if (event.data.sale_bill_break_itemofsale_prod_break_headers) {
					...
					if(event.data.sale_bill_break_itemofsale_prod_break_headers.length!=0){
						for(var i=0;i<event.data.sale_bill_break_itemofsale_prod_break_headers.length;i++){
						sum1+=parseFloat(event.data.sale_bill_break_itemofsale_prod_break_headers[i].package_qty||0)*parseFloat(event.data.sale_bill_break_itemofsale_prod_break_headers[i].package_tj||0)
						$scope.data.currItem.summary1=sum1;
					    }
					}else{
						$scope.data.currItem.summary1=0;
					}
	...
	//分体机选择返包批次编码
	$scope.back_erp_code23 = function () {
        $scope.FrmInfo = {
			is_custom_search:true,
            title: "ERP编码查询",
            thead: [{
	    ...
	    data[index].back_erp_code = result.item_no; 
			
	$scope.data.currItem.sale_prod_h_lineofsale_prod_headers[rowidx].sale_prod_lineofsale_prod_h_lines=data;

	$scope.options_23.api.setRowData(data);
        });
    };
        //返包数量 
        $scope.back_qty23 = function (e) {
            ...
            var nodes = $scope.options_23.api.getModel().rootNode.childrenAfterSort;
				...
				//(nodes[i].data.back_qty == undefined || nodes[i].data.back_qty == "") &&
				//index!= i &&
			if (e.currentTarget.value) {
				for (var i = 0; i < nodes.length; i++) {
					nodes[i].data.back_qty = e.currentTarget.value; 
					$scope.options_23.api.refreshCells(nodes, ["back_qty"], true);				
				}
			} else {
				for (var i = 0; i < nodes.length; i++) {
					nodes[i].data.back_qty = "";
					$scope.options_23.api.refreshCells(nodes, ["back_qty"], true);				
				}
			}
            $scope.options_23.api.stopEditing();
			$scope.options_23.api.setFocusedCell(index, 'back_qty');

            $scope.options_23.api.startEditingCell({rowIndex: index,colKey: 'back_qty', keyPress:2});
        };
        ...
        $scope.addline21 = function () {
            ...
            BasemanService.open(CommonPopController, $scope, "", "lg").result.then(function (items) {
                if (items.length) {
                    ...
                    $scope.data.currItem.sale_prod_h_lineofsale_prod_headers = data;
                }
                $scope.options_21.api.setRowData($scope.data.currItem.sale_prod_h_lineofsale_prod_headers);
                for (var i = 0; i < $scope.data.currItem.sale_prod_h_lineofsale_prod_headers.length; i++) {
                    var h_item = $scope.data.currItem.sale_prod_h_lineofsale_prod_headers[i].sale_prod_lineofsale_prod_h_lines;
                    if (h_item == undefined) {
                        h_item = [];
				 ...
				 //交期拆分
                  for (var i = 0; i < $scope.data.currItem.sale_prod_h_lineofsale_prod_headers.length; i++) {
                                var object = {};
                                for (name in $scope.data.currItem.sale_prod_h_lineofsale_prod_headers[i]) {
                                    if (name != "sale_prod_lineofsale_prod_h_lines") {
                                        object[name] = $scope.data.currItem.sale_prod_h_lineofsale_prod_headers[i][name];
                                    }
                                }
                                $scope.data.currItem.sale_prod_h_lineofsale_prod_headers[i].sale_prod_h_detlofsale_prod_h_lines = [];
                                $scope.data.currItem.sale_prod_h_lineofsale_prod_headers[i].sale_prod_h_detlofsale_prod_h_lines.push(object);
                 }			 
            });
        };
		...
		if(rowidx>0){
		  ...
		  $scope.options_22.api.setRowData($scope.data.currItem.sale_prod_h_lineofsale_prod_headers[rowidx].sale_prod_h_detlofsale_prod_h_lines);
          $scope.options_23.api.setRowData($scope.data.currItem.sale_prod_h_lineofsale_prod_headers[rowidx].sale_prod_lineofsale_prod_h_lines);
          $scope.options_28.api.setRowData($scope.data.currItem.sale_prod_h_lineofsale_prod_headers[rowidx].sale_prod_mause_lineofsale_prod_h_lines);			  
		}else{
		  ...
		  var colKey= $scope.options_21.api.getFocusedCell().column.colId
		  $scope.options_21.api.setFocusedCell(rowidx,colKey);
		  $scope.options_22.api.setRowData($scope.data.currItem.sale_prod_h_lineofsale_prod_headers[rowidx+1].sale_prod_h_detlofsale_prod_h_lines);
          $scope.options_23.api.setRowData($scope.data.currItem.sale_prod_h_lineofsale_prod_headers[rowidx+1].sale_prod_lineofsale_prod_h_lines);
          $scope.options_28.api.setRowData($scope.data.currItem.sale_prod_h_lineofsale_prod_headers[rowidx+1].sale_prod_mause_lineofsale_prod_h_lines);	  
		  }
		}		
		//删除配件行
    ...
    document.getElementById('divPIconfBgIframe').src = pcsurl;		
            ...
            BasemanService.openFrm("views/Pop_copy_Line.html", PopCopyLineController, $scope)
                .result.then(function (result) {
                    ...
                    if ($scope.options_21.api.getFocusedCell()) {
                        ...
                        $scope.data.currItem.sale_prod_h_lineofsale_prod_headers[$scope.options_21.api.getFocusedCell().rowIndex].sale_prod_h_detlofsale_prod_h_lines = data;
                    } else {
                        $scope.data.currItem.sale_prod_h_lineofsale_prod_headers[0].sale_prod_h_detlofsale_prod_h_lines = data;
                    }
                    $scope.options_22.api.setRowData(data);
                    ...
                    .result.then(function (items) {
                       ...
                       /** //加到订单明细的易损件中
                        if ($scope.is_ys()) {
                            ...
                            data1[j].sale_amt = parseFloat(data1[j].sale_amt || 0) + temp.sale_amt;
                            data1[j].price = parseFloat(data1[j].price || 0) + temp.price;
                            data1[j].sale_prod_lineofsale_prod_h_lines[0].sale_amt = parseFloat(data1[j].sale_prod_lineofsale_prod_h_lines[0].sale_amt || 0)
                                + temp.sale_prod_lineofsale_prod_h_lines[0].sale_amt;
                            data1[j].sale_prod_lineofsale_prod_h_lines[0].price = parseFloat(data1[j].sale_prod_lineofsale_prod_h_lines[0].price || 0)
                                + temp.sale_prod_lineofsale_prod_h_lines[0].price;
        ...
        $scope.addls24 = function () {
            ...
            BasemanService.open(CommonPopController, $scope).result.then(function (result) {
                ...
                BasemanService.RequestPost("sale_prod_header", "getprodfreepart", postdata)
                    .then(function (data) {
                        var temp=$scope.gridGetData("options_21")[index];
                         for(var i=0;i<data.sale_prod_line_partofsale_prod_headers.length;i++){
							 data.sale_prod_line_partofsale_prod_headers[i].item_line_seq=parseInt(temp.seq);
						 }						
                        $scope.options_24.api.setRowData(data.sale_prod_line_partofsale_prod_headers);
                        $scope.data.currItem.sale_prod_line_partofsale_prod_headers = data.sale_prod_line_partofsale_prod_headers
                    })
            });
        };
        //引入机型必选配件
        $scope.addbx24 = function () {
            ...
            if ($scope.options_21.api.getFocusedCell()) {
				...
				BasemanService.notice("请选定指定对应机型", "alter-warning")
                return;
			}
            //var rowidx = $scope.options_21.api.getFocusedCell().rowIndex;
            var data1 = [];
...
//		postdata.item_code=node[rowidx].data.item_code;
        ...
        $scope.allcancel43 = function () {
            ...
            ds.dialog.confirm("是否确认取消整单包装方案？", function () {
                ...
                $scope.options_43.api.deselectAll("options_43");
                // $scope.newWindow.close();
            });
        };
        //计算包装方案
    ...
    /******************弹出框区域****************************/
    {
        ...
        $scope.PI_search = function () {
            ...
            $scope.FrmInfo = {
                classid: "sale_pi_header",
                postdata: {},
                sqlBlock: "stat = 5 and nvl(Sale_PI_Header.Is_Prod_Comp,0)<>2 and nvl(is_closed,1) = 1 and nvl(Sale_PI_Header.bill_flag,0) not in (2,3) and ( exists (select 1 from sale_pi_item_H_line l,( select sum(ll.prod_qty) donocheckqty,hh.pi_id,ll.pi_h_id from sale_prod_header hh,sale_prod_H_line ll where hh.prod_id=ll.prod_id and hh.stat < 5 group by hh.pi_id,ll.pi_h_id ) b where l.pi_id=sale_pi_header.pi_id and l.pi_id=b.pi_id(+) and l.pi_h_id=b.pi_h_id(+) and l.line_type <> 4 and l.qty > (l.proded_qty + nvl(b.donocheckqty,0))) or exists ( select 1 from sale_pi_item_H_line l,( select sum(ll.part_qty) donocheckqty,hh.pi_id,ll.pi_h_id from sale_prod_header hh,sale_prod_H_line ll where hh.prod_id=ll.prod_id and hh.stat <= 5 group by hh.pi_id,ll.pi_h_id ) b where l.pi_id=sale_pi_header.pi_id and l.pi_id=b.pi_id(+) and l.pi_h_id=b.pi_h_id(+) and l.line_type =4 and case when l.part_qty =0 then 1 else l.part_qty end > (nvl(b.donocheckqty,0))) )",
                searchlist: ["pi_no", "item_type_name", "pi_date", "item_type_name", "amt_total", "org_name", "cust_name"],
            };
            BasemanService.open(CommonPopController, $scope, "", "lg")
                .result.then(function (result) {
                    ...
                    $scope.data.currItem.exchangerate = $scope.data.currItem.exchange_rate;
                    $scope.data.currItem.pi_amt = parseFloat(tmpe.amt_total);
                    $scope.data.currItem.pi_part_rate = tmpe.amt_gathering;
                    $scope.data.currItem.tttpje = tmpe.total_tt;
                    $scope.data.currItem.amt9 = tmpe.amt_total;
                    $scope.data.currItem.oatpje = tmpe.total_oa;
                    $scope.data.currItem.lctpje = tmpe.total_lc;
                    $scope.data.currItem.cust_level = tmpe.cust_level;
                    if ($scope.data.currItem.pi_id == 0) {
                        return;
                    }
                    ;
                    if ($scope.data.currItem.creator == tmpe.creator) {
                        $scope.data.currItem.creator = "";
                    }
                    if ($scope.data.currItem.create_time == tmpe.create_time) {
                        $scope.data.currItem.create_time = "";
                    }
                    if ($scope.data.currItem.note != "") {
                        $scope.data.currItem.note = tmpe.note;
                    } else {
                        $scope.data.currItem.note = ""
                    }
                    ;
                    if ($scope.data.currItem.prod_no != "") {
                        $scope.data.currItem.prod_id = tmpe.prod_id;
                        $scope.data.currItem.prod_no = tmpe.prod_no;
                    }
                    $scope.data.currItem.wfid = 0;
                    $scope.data.currItem.wfflag = 0;
                    $scope.data.currItem.pre_ship_date = "";
                    $scope.data.currItem.latest_ship_date = "";
                    $scope.data.currItem.is_ctrl = 1;
                    $scope.data.currItem.zb_amt = 0;
					//生产定金为空
					$scope.data.currItem.amt1="";
					$scope.data.currItem.contract_subscrp="";
					$scope.data.currItem.amt2="";
					$scope.data.currItem.amt3="";
					$scope.data.currItem.tttpje="";
					$scope.data.currItem.amt4="";
					$scope.data.currItem.amt5="";
					$scope.data.currItem.amt6="";
					$scope.data.currItem.amt7="";
					$scope.data.currItem.oatpje="";
					$scope.data.currItem.amt9="";
					$scope.data.currItem.amt8="";
//               HczyCommon.copyobj($scope.data.currItem,tmpe);
//               tmpe.sales_user_id=tmpe.creator;

                    $scope.data.currItem.stat = 1;
                    var postdata = {pi_id: parseInt(result.pi_id)};

                    BasemanService.RequestPost("sale_pi_header", "getpiitemline", postdata)
                        .then(function (data) {
                            if (tmpe.pi_no == "") {
                                return;
                            }
                            ...
                            $scope.data.currItem.sale_prod_line_partofsale_prod_headers = data.sale_pi_item_partofsale_pi_headers;
                            if (data.customer_brandofcustomers.length > 0)$scope.data.currItem.brand_name = data.customer_brandofcustomers[0].brand_name;
                            var sale_prod = [];
                            for (var i = 0; i < data.sale_pi_item_h_lineofsale_pi_headers.length; i++) {
                                var obj = new Object();
                                obj.pi_qty = data.sale_pi_item_h_lineofsale_pi_headers[i].qty;
                                obj.prod_qty = data.sale_pi_item_h_lineofsale_pi_headers[i].pi_qty - data.sale_pi_item_h_lineofsale_pi_headers[i].proded_qty - data.sale_pi_item_h_lineofsale_pi_headers[i].donocheckqty;
                                if (data.sale_pi_item_h_lineofsale_pi_headers[i].line_type) {
                                    obj.prod_qty = 1;
                                }
                                obj.settle_price = data.sale_pi_item_h_lineofsale_pi_headers[i].p4
                                obj.already_in_qty = 0;
                                obj.already_ship_qty = 0;
                                obj.warned_qty = 0;
                                obj.actual_out_qty = 0;
                                obj.sale_prod_lineofsale_prod_h_lines = data.sale_pi_item_h_lineofsale_pi_headers[i].sale_pi_item_lineofsale_pi_item_h_lines
                                obj.item_h_id = data.sale_pi_item_h_lineofsale_pi_headers[i].item_h_id;
                                obj.item_h_code = data.sale_pi_item_h_lineofsale_pi_headers[i].item_h_code;
                                obj.item_h_name = data.sale_pi_item_h_lineofsale_pi_headers[i].item_h_name;
                                obj.line_type = data.sale_pi_item_h_lineofsale_pi_headers[i].line_type;
                                obj.cust_item_name = data.sale_pi_item_h_lineofsale_pi_headers[i].cust_item_name;
                                //obj.brand_name = data.sale_pi_item_h_lineofsale_pi_headers[i].brand_name;
                                obj.proded_qty = data.sale_pi_item_h_lineofsale_pi_headers[i].proded_qty;
                                obj.brand_name = data.sale_pi_item_h_lineofsale_pi_headers[i].brand_name;
                                obj.area_names = data.sale_pi_item_h_lineofsale_pi_headers[i].area_names;
                                obj.area_ids = data.sale_pi_item_h_lineofsale_pi_headers[i].area_ids;
                                obj.prod_qty = (parseInt(data.sale_pi_item_h_lineofsale_pi_headers[i].qty) - parseInt(obj.proded_qty));
                                obj.pi_h_id = data.sale_pi_item_h_lineofsale_pi_headers[i].pi_h_id;
                                obj.pi_id = data.sale_pi_item_h_lineofsale_pi_headers[i].pi_id;
                                obj.price = data.sale_pi_item_h_lineofsale_pi_headers[i].price;
                                obj.pro_type = data.sale_pi_item_h_lineofsale_pi_headers[i].pro_type;
                                sale_prod.push(obj)
                            }
                            ;

                            // var json_items = [].concat($scope.data.currItem.sale_prod_h_lineofsale_prod_headers);
                            $scope.data.currItem.sale_prod_h_lineofsale_prod_headers = sale_prod;
                            //订单明细
                            $scope.options_21.api.setRowData($scope.data.currItem.sale_prod_h_lineofsale_prod_headers);
                            $scope.data.currItem.sale_prod_h_lineofsale_prod_headers = sale_prod;
                            //交期拆分
                            ...
                            var data2 = $scope.data.currItem.sale_prod_h_lineofsale_prod_headers[0].sale_prod_h_detlofsale_prod_h_lines;
                            $scope.options_22.api.setRowData(data2);
                            //分体机
                            $scope.options_23.api.setRowData($scope.data.currItem.sale_prod_h_lineofsale_prod_headers[0].sale_prod_lineofsale_prod_h_lines);
                            //配件清单
                            $scope.options_24.api.setRowData(data.sale_pi_item_partofsale_pi_headers);
                            $scope.data.currItem.sale_prod_line_partofsale_prod_headers = data.sale_pi_item_partofsale_pi_headers;
                            //打散-包装方案
                            $scope.options_41.api.setRowData(data.sale_pi_break_headerofsale_pi_headers);
                            $scope.data.currItem.sale_prod_break_headerofsale_prod_headers = data.sale_pi_break_headerofsale_pi_headers;
                            for (var i = 0; i < $scope.data.currItem.sale_prod_break_headerofsale_prod_headers.length; i++) {
                                //打散规则
                                $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[i].sale_prod_break_lineofsale_prod_break_headers = $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[i].sale_pi_break_lineofsale_pi_break_headers;
                                delete    $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[i].sale_pi_break_lineofsale_pi_break_headers;
                                //包装方案
                                $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[i].sale_prod_break_itemallofsale_prod_break_headers = $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[i].sale_pi_break_itemallofsale_pi_break_headers;
                                delete    $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[i].sale_pi_break_itemallofsale_pi_break_headers;
                                //辅料用料汇总
                                $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[i].sale_bill_break_itemofsale_prod_break_headers = $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[i].sale_bill_break_itemofsale_pi_break_headers;
                                delete    $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[i].sale_bill_break_itemofsale_pi_break_headers;
                            }
                            var temp = $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[0];
                            if (temp && temp.sale_prod_break_lineofsale_prod_break_headers) {
                                $scope.options_42.api.setRowData($scope.data.currItem.sale_prod_break_headerofsale_prod_headers[0].sale_prod_break_lineofsale_prod_break_headers);
                            }
                            if (temp && temp.sale_prod_break_itemallofsale_prod_break_headers) {
                                $scope.options_43.api.setRowData($scope.data.currItem.sale_prod_break_headerofsale_prod_headers[0].sale_prod_break_itemallofsale_prod_break_headers);
                            }
                            if (temp && temp.sale_bill_break_itemofsale_prod_break_headers) {
                                $scope.options_44.api.setRowData($scope.data.currItem.sale_prod_break_headerofsale_prod_headers[0].sale_bill_break_itemofsale_prod_break_headers);
                            }
                            $scope.options_46.api.setRowData(data.sale_pi_item_bom_lineofsale_pi_headers);
                            $scope.data.currItem.sale_prod_bom_lineofsale_prod_headers = data.sale_pi_item_bom_lineofsale_pi_headers;
                            $scope.options_47.api.setRowData(data.sale_pi_item_lineofsale_pi_header);
                            //$scope.data.currItem.sale_pi_item_lineofsale_pi_headers = data.sale_pi_item_lineofsale_pi_header;

                            //产品配置 当前机型
                            $scope.data.currItem.sale_prod_line_conf2ofsale_prod_headers = data.sale_prod_line_conf2ofsale_prod_headers;
                            $scope.options_31.api.setRowData(data.sale_prod_line_conf2ofsale_prod_headers);
                            //产品配置 所有机型
                            $scope.data.currItem.sale_prod_line_confofsale_prod_headers = data.sale_prod_line_confofsale_prod_headers;
                            $scope.options_32.api.setRowData(data.sale_prod_line_confofsale_prod_headers);
                            //印刷件
                            var data = [];
                            for (var i = 0; i < $scope.data.currItem.sale_prod_h_lineofsale_prod_headers.length; i++) {
                                for (var j = 0; j < $scope.data.currItem.sale_prod_h_lineofsale_prod_headers[i].sale_prod_lineofsale_prod_h_lines.length; j++) {
                                    if ((parseInt($scope.data.currItem.sale_prod_h_lineofsale_prod_headers[i].sale_prod_lineofsale_prod_h_lines[j].pro_type)) != 4) {
                                        data.push($scope.data.currItem.sale_prod_h_lineofsale_prod_headers[i].sale_prod_lineofsale_prod_h_lines[j]);
                                    }
                                }
                            }
                            $scope.data.currItem.sale_prod_lineofsale_prod_headers = data;
                            $scope.options_51.api.setRowData($scope.data.currItem.sale_prod_lineofsale_prod_headers);
    ...
    /***********************网格定义**************************/
    {
        ...
        $scope.columns_21 = [
            ...
            },		
            {
                ...
                headerName: "本次排产数量", field: "prod_qty", editable: true, filter: 'set', width: 120,
                cellEditor: "文本框",
        ...
        $scope.columns_23 = [
            ...
            },
            {
				...
				headerName: "返包ERP编码", field: "back_erp_code", editable: true, filter: 'set', width: 115,
				cellEditor: "弹出框",
				action: $scope.back_erp_code23,
				enableRowGroup: true,
				enablePivot: true,
				enableValue: true,
				floatCell: true 
				...
				headerName: "返包数量", field: "back_qty", editable: true, filter: 'set', width: 90,
				cellEditor: "整数框", 
				cellchange: $scope.back_qty23,
				enableRowGroup: true,
				enablePivot: true,
				enableValue: true,
				floatCell: true
        ...
        $scope.columns_43 = [
            ...
            },
            {
                ...
                headerName: "row_flag", field: "row_flag", editable: true, filter: 'set', width: 70,
                cellEditor: "文本框",
				cellRenderer: function (params) {
                    return colorRenderer(params)
                },
    ...
    $scope.wfstart_before = function () {
        ...
        var msg = [];
		var errorlist=[];
        var data = $scope.data.currItem.sale_prod_line_partofsale_prod_headers;
        /**if (data == undefined) {
        ...
        /**for (var i = 0; i < $scope.data.currItem.sale_prod_break_headerofsale_prod_headers.length; i++) {
            for (var j = 0; j < $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[i].sale_bill_break_itemofsale_prod_break_headers.length; j++) {
                for(name in $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[i].sale_bill_break_itemofsale_prod_break_headers[j]){
					if(name!="item_cust_code"&&name!="item_cust_desc"&&name!="pocustno"){
						if($scope.data.currItem.sale_prod_break_headerofsale_prod_headers[i].sale_bill_break_itemofsale_prod_break_headers[j][name]==""){
        ...
        var obj = BasemanService.RequestPost("sale_prod_header", "cimmitcheck", {prod_id: $scope.data.currItem.prod_id})
        return obj;
    ...
    $scope.save_before = function () {
        ...
        for (var i = 0; i < data.length; i++) {
            if (parseInt(data[i].line_type) == 4) {
                ...
                data1.push(data[i]);
                data1[0].seq = 1;
                $scope.data.currItem.sale_prod_packageofsale_prod_headers = data1;
            }
        }
        var postdata = $scope.data.currItem;
        if ($scope.data.currItem.stat > 1) {
			postdata.sale_prod_break_itemallofsale_prod_headers=[];
			postdata.sale_prod_h_detlofsale_prod_headers=[];
		    for(var i=0;i<postdata.sale_prod_break_headerofsale_prod_headers.length;i++){
				for(var j=0;j<postdata.sale_prod_break_headerofsale_prod_headers[i].sale_prod_break_itemallofsale_prod_break_headers.length;j++){
				postdata.sale_prod_break_itemallofsale_prod_headers.push(postdata.sale_prod_break_headerofsale_prod_headers[i].sale_prod_break_itemallofsale_prod_break_headers[j]);
				}
			}
			
			for(var i=0;i<postdata.sale_prod_h_lineofsale_prod_headers.length;i++){
				for(var j=0;j<postdata.sale_prod_h_lineofsale_prod_headers[i].sale_prod_h_detlofsale_prod_h_lines.length;j++){
				postdata.sale_prod_h_detlofsale_prod_headers.push(postdata.sale_prod_h_lineofsale_prod_headers[i].sale_prod_h_detlofsale_prod_h_lines[j]);
				}
			}
            postdata.flag = 2;
        } else {
        ...
        for (var i = 0; i < $scope.data.currItem.sale_prod_line_confofsale_prod_headers.length; i++) {
            $scope.data.currItem.sale_prod_line_confofsale_prod_headers[i].seq = parseInt(i + 1);
        }
        for (var i = 0; i < $scope.data.currItem.sale_prod_line_conf2ofsale_prod_headers.length; i++) {
            $scope.data.currItem.sale_prod_line_conf2ofsale_prod_headers[i].seq = parseInt(i + 1);
        }
    };
    /*********************界面初始化************************/
...
//加载控制器
basemanControllers
    .controller('sale_prod_headerEdit', sale_prod_headerEdit);
