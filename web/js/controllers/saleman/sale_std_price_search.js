...
function sale_std_price_search($scope, $location, $rootScope, $modal, notify, $timeout, BasemanService, $state, localeStorageService, FormValidatorService) {
    //继承基类方法
    sale_std_price_search = HczyCommon.extend(sale_std_price_search, ctrl_bill_public);
    sale_std_price_search.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
        name: "sale_std_priceapply_header",
        FrmInfo: {},
        grids: [{optionname: 'options_13', idname: 'sale_std_priceapply_line2ofsale_std_priceapply_headers'}]
    };
    //查询
    $scope.search = function () {
        ...
        BasemanService.RequestPost("sale_std_priceapply_header", "getitemlist", postdata)
            .then(function (data) {
                ...
                $scope.options_13.api.setRowData(data.sale_std_priceapply_line2ofsale_std_priceapply_headers);
            });
    };
    /***************************弹出框***********************/
    //整机编码
    ...
    $scope.columns_13 = [
        {
            ...
            headerName: "标准材料成本", field: "std_price", editable: false, filter: 'set', width: 150,
            cellEditor: "浮点框",
            enableRowGroup: true,
            enablePivot: true,
            enableValue: true,
...
//加载控制器
basemanControllers
    .controller('sale_std_price_search', sale_std_price_search);
