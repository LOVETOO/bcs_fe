...
function sale_prod_header_pjEdit($scope, $location, $rootScope, $modal, notify, $timeout, BasemanService, $state, localeStorageService, FormValidatorService) {
    //继承基类方法
    sale_prod_header_pjEdit = HczyCommon.extend(sale_prod_header_pjEdit, ctrl_bill_public);
    sale_prod_header_pjEdit.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
        name: "sale_prod_header",
        key: "prod_id",
        wftempid: "10016",
        FrmInfo: {sqlBlock:"1=1 and  stat = 5  and nvl(package_over,0) <> 2"},
        postdata: {
            flag: 10
        },
        grids: [
...
//		postdata.item_code=node[rowidx].data.item_code;
        ...
        $scope.cancel43 = function () {
            ...
            data1.push(data[0]);
            var rowidx = $scope.options_41.api.getFocusedCell().rowIndex;
            $scope.data.currItem.sale_prod_break_headerofsale_prod_headers[rowidx].sale_break_lineofsale_break_headers = data1;
        ...
        $scope.addline52 = function () {
            ...
            BasemanService.open(CommonPopController, $scope).result.then(function (result) {
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    if (request.pass) {
                        tempobj.material = request.data.material;
                        tempobj.part_location = request.data.loc;
                        tempobj.part_content = request.data.content;
                    }
                    ;
                    data.push(tempobj)
                }
...
//               tmpe.sales_user_id=tmpe.creator;
    ...
    //控制只读
    $scope.refresh_after = function () {
        if ($scope.data.currItem.package_over != 2&&$scope.data.currItem.stat==5) {
            $scope.columns_25[$scope.getIndexByField('columns_25', 'pack_qty')].editable = true;
			$scope.updateallColumns();
        }
    }
    /*********************保存校验区域************************/
    $scope.validate = function () {
        if ($rootScope.$state.$current.self.name == "crmman.sale_prod_header_pjEdit"){//TT到款
            if (Number($scope.data.currItem.sale_prod_packageofsale_prod_headers[0].pack_qty || 0) == 0) {
                BasemanService.notice('打包箱数为0,是否确认转箱?', "alert-warning");
                return;
            } else {
                $scope.data.currItem.package_over=2;
            }
        }
        var line_qty = 0;
...
//加载控制器
basemanControllers
    .controller('sale_prod_header_pjEdit', sale_prod_header_pjEdit);
