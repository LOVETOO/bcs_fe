...
function sale_ship_warn_headerEdit($scope, $location, $http, $rootScope, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService, HistoryDataService) {
                    ...
                    for (var i = 0; i < items.length; i++) {
                        ...
                        for (var i = 0; i < itemlines.length; i++) {
                            if (itemlines[i].pi_id == items[i].pi_id) {//pi_id相同的数据
                                ...
                                itemlines[i].lc_no = items[i].lc_no;
                            }
                        }
                        /*对单证信息增加信用证行*/
                        if (!is_to_aoptions) {
            ...
            $scope.cntcolumns = [
                {
                    ...
                    headerName: "船证明特殊条款", field: "vessel_certificate", editable: true, filter: 'set', width: 125,
                    cellEditor: "文本框",
                    enableRowGroup: true,
                    enablePivot: true,
                    enableValue: true,
        ...
        //aoptions,根据是否是空白行，判断列的可编辑
        {
                ...
                BasemanService.open(CommonPopController, $scope, "", "lg")
                    ...
                    if (items.length > 0) {
                        ...
                        for (var i = 0; i < items.length; i++) {
                            ...
                            items[i].pack_style = Number(items[i].qty);
                            items[i].is_needsj = 2;
                            //将所需资料写到dg上
                            items[i].bg_name = '';
            ...
            $scope.add_wh = function () {
                $scope.FrmInfo = {
                    ...
                    sqlBlock: "cust_id=" + (($scope.data.currItem.cust_id) || 0) + " or  nvl(cust_id,0)=0",
                    classid: "po_inventory",
                };
                BasemanService.open(CommonPopController, $scope, "", "lg")
                    .result.then(function (result) {
                    var postdata = {inv_id: result.inv_id}
                    BasemanService.RequestPost("po_inventory", "select", postdata)
                        .then(function (data1) {
                            var data = $scope.gridGetData("aoptions");
                            for(var i=0;i<data.length;i++){
                                if(data[i].po_item_id==data1.po_item_id){
                                    BasemanService.notice("该外购件已存在不可重复添加，如有需要增加请修改出货预告数量!")
                                    return;
                                }
                            }
                            var itemTemp={};
                            ...
                            itemTemp.qty=0;
                            itemTemp.pro_type=5;
                            itemTemp.item_name=data1.po_item_name;
                            itemTemp.item_code=data1.po_item_code;
                            itemTemp.item_id=data1.po_item_id;
                            $scope.gridAddItem('aoptions', itemTemp);
                        });
                });
            }
    ...
    $scope.partSummary = function () {
        $timeout(function () {
            ...
            $scope.data.currItem.tl_pack_style = 0;
            //总体积
            for (var i = 0; i < data.length; i++) {
                ...
                if (parseFloat(data[i].pack_styl) != undefined) {
                    $scope.data.currItem.tl_pack_style += parseFloat(data[i].pack_style || 0);
                }
            }
    ...
    /**导出excel*/
    {
        ...
        $scope.exportjy1 = function (e) {
            var adPopup = window.open('about:blank', '_blank', 'height=100, width=400, top=0,left=0, toolbar=no,menubar=no, scrollbars=no, resizable=no,location=no, status=no');
            if (!$scope.data.currItem.warn_id) {
                BasemanService.notice("未识别单据", "alert-info");
                return;
            }
            BasemanService.RequestPost("sale_ship_warn_header", "exporttoexceljy1", {'warn_id': $scope.data.currItem.warn_id})
                .then(function (data) {
        ...
        $scope.exportjy2 = function () {
            ...
            BasemanService.RequestPost("sale_ship_warn_header", "exporttoexceljy2", {'warn_id': $scope.data.currItem.warn_id})
                .then(function (data) {
                    if (data.docid != undefined && Number(data.docid) != 0) {
                        var fileurl = '/downloadfile.do?docid=' + data.docid + '&loginguid=' + window.strLoginGuid + '&t=' + new Date().getTime();
        ...
        $scope.exportjy3 = function () {
            ...
            if ($scope.cntoptions) {
                postdata.sale_ship_warn_lc_lineofsale_ship_warn_headers = $scope.selectGridGetData("cntoptions");
            }
            BasemanService.RequestPost("sale_ship_warn_header", "exporttoexceljy3", postdata)
                .then(function (data) {
                    if (data.docid != undefined && Number(data.docid) != 0) {
                        var fileurl = '/downloadfile.do?docid=' + data.docid + '&loginguid=' + window.strLoginGuid + '&t=' + new Date().getTime();
        ...
        $scope.exportjy4 = function () {
            ...
            BasemanService.RequestPost("sale_ship_warn_header", "exporttoexceljy4", {'warn_id': $scope.data.currItem.warn_id})
                .then(function (data) {
                    if (data.docid != undefined && Number(data.docid) != 0) {
                        var fileurl = '/downloadfile.do?docid=' + data.docid + '&loginguid=' + window.strLoginGuid + '&t=' + new Date().getTime();
                        window.open(fileurl, '_blank', 'height=100, width=400, top=0,left=0, toolbar=no,menubar=no, scrollbars=no, resizable=no,location=no, status=no');
        ...
        $scope.exportjy5 = function () {
            ...
            BasemanService.RequestPost("sale_ship_warn_header", "exporttoexceljy5", {'warn_id': $scope.data.currItem.warn_id})
                .then(function (data) {
                    if (data.docid != undefined && Number(data.docid) != 0) {
                        var fileurl = '/downloadfile.do?docid=' + data.docid + '&loginguid=' + window.strLoginGuid + '&t=' + new Date().getTime();
                        window.open(fileurl, '_blank', 'height=100, width=400, top=0,left=0, toolbar=no,menubar=no, scrollbars=no, resizable=no,location=no, status=no');
        ...
        $scope.exportjy6 = function () {
            ...
            BasemanService.RequestPost("sale_ship_warn_header", "exporttoexceljy6", {'warn_id': $scope.data.currItem.warn_id})
                .then(function (data) {
                    if (data.docid != undefined && Number(data.docid) != 0) {
                        var fileurl = '/downloadfile.do?docid=' + data.docid + '&loginguid=' + window.strLoginGuid + '&t=' + new Date().getTime();
                        window.open(fileurl, '_blank', 'height=100, width=400, top=0,left=0, toolbar=no,menubar=no, scrollbars=no, resizable=no,location=no, status=no');
    ...
    $scope.validate = function () {
        ...
        for(var i;i<data.length;i++){
            if(data[i].pro_type==5){
                var postdata = {flag:1,po_item_id: data[i].po_item_id||"",}
                var a=BasemanService.RequestPostNoWait("po_inventory", "select", postdata);
                if(!a.pass){
                    return;
                }
                var data1=a.data;
                if(data[i].qty>data1.po_inventorys[0].canuse_qty){
                    msg.push("产品信息第"+(i+1)+"行外购件的发货数量大于库存数量" + linemsg);
                }
            }
        }
        if (msg.length > 0) {
    ...
    $scope.wfproc_warn = function () {
        if ($scope.data.currItem.currprocname != "散件包排柜") {
            return true;
        }
        ...
        for (var i = 0; i < $scope.data.currItem.sale_ship_warn_line_sumofsale_ship_warn_headers.length; i++) {
            obj = $scope.data.currItem.sale_ship_warn_line_sumofsale_ship_warn_headers[i];
            if (obj.saleorder_type == 5 && obj.line_type == 4 && obj.sy_stat != 2 && obj.add_row == 2) {
                if (msg.length == 0) {
                    msg = obj.cust_item_name;
                }
                msg += "," + obj.cust_item_name;
            }
        }
        var strArr = [];
        if (msg.length > 0) {
            msg = "以下机型:" + msg + "的重量来源不是称重,不能提交,请联系散件技术支持!";
            strArr.push(msg);
        }
        var warnqty = 0, pqty1 = 0, mtqty = 0, sumid = 0, qty = 0, pqty2 = 0;
        for (var i = 0; i < $scope.data.currItem.sale_ship_warn_line_sumofsale_ship_warn_headers.length; i++) {
            obj = $scope.data.currItem.sale_ship_warn_line_sumofsale_ship_warn_headers[i];
            qty = 0;
            pqty2 = 0;
            for (var j = 0; j < $scope.data.currItem.sale_ship_warn_lineofsale_ship_warn_headers.length; j++) {
                obj1 = $scope.data.currItem.sale_ship_warn_lineofsale_ship_warn_headers[j];
                if (obj.warn_sum_id == obj1.warn_sum_id) {
                    qty += Number(obj1.qty || 0);
                    pqty2 += Number(obj1.pack_style || 0);
                }
            }
            if (obj.qty != qty || obj.pack_style != pqty2) {
                msg = "商检批号:" + obj.inspection_batchno + "-产品["
                    + obj.cust_spec + "]对应产品明细中的数量:" + obj.qty + ",请修改!";
                strArr.push(msg);
            }
        }
        for (var i = 0; i < $scope.data.currItem.sale_ship_warn_box_lineofsale_ship_warn_headers.length; i++) {
            obj = $scope.data.currItem.sale_ship_warn_box_lineofsale_ship_warn_headers[i];
            if (obj.sale_ship_warn_line_sumofsale_ship_warn_box_lines.length == 0) {
                strArr.push("第" + obj.seq + "的货柜没有使用，请删除!")
            }
        }
        //产品明细与排柜明细上的总体积、毛重、净重应相等
        var totalnw2 = 0, totalgw2 = 0, totaltj2 = 0;
        for (var i = 0; i < $scope.data.currItem.sale_ship_warn_line_sumofsale_ship_warn_headers.length; i++) {
            obj = $scope.data.currItem.sale_ship_warn_line_sumofsale_ship_warn_headers[i];
            totalnw2 = 0;
            totalgw2 = 0;
            ...
            for (var j = 0; j < $scope.data.currItem.sale_ship_warn_lineofsale_ship_warn_headers.length; j++) {
                obj1 = $scope.data.currItem.sale_ship_warn_lineofsale_ship_warn_headers[j];
                if (obj.warn_sum_id == obj1.warn_sum_id) {
                    totalnw2 += Number(obj1.total_nw || 0);
                    totalgw2 += Number(obj1.total_gw || 0);
                    totaltj2 += Number(obj1.total_tj || 0);
                }
            }
            if (Math.abs(Number(obj.total_nw || 0) - totalnw2) > 0.000001) {
                msg = "商检批号:" + obj.inspection_batchno + "在产品明细的总净重与排柜明细的总净重不一致!"
                strArr.push(msg);
            }
            if (Math.abs(Number(obj.total_gw || 0) - totalgw2) > 0.000001) {
                msg = "商检批号:" + obj.inspection_batchno + "在产品明细的总毛重与排柜明细的总毛重不一致!"
                strArr.push(msg);
            }
            if (Math.abs(Number(obj.total_tj || 0) - totaltj2) > 0.000001) {
                msg = "商检批号:" + obj.inspection_batchno + "在产品明细的总体积与排柜明细的总体积不一致!"
                strArr.push(msg);
            }
        }
        if (strArr.length > 0) {
            BasemanService.notice(strArr, "alert-warning");
            return false;
        }
        //以上为提示报错，不能提交的
        /**下面为提示，可以继续提交
        ...
        for (var i = 0; i < $scope.data.currItem.sale_ship_warn_line_sumofsale_ship_warn_headers.length; i++) {
            obj = $scope.data.currItem.sale_ship_warn_line_sumofsale_ship_warn_headers[i];
            if (obj.saleorder_type != 5 && obj.saleorder_type != 6) {
                continue;
            }
            if (obj.pro_type == 4) {
                continue;
            ...
            for (var j = 0; j < $scope.data.currItem.sale_ship_warn_package_lineofsale_ship_warn_headers.length; j++) {
                obj1 = $scope.data.currItem.sale_ship_warn_package_lineofsale_ship_warn_headers[j];
                if (obj.erp_code == obj1.erp_code && (obj1.ref_box_no != undefined && obj1.ref_box_no != "")) {
                    isexists = true;
                    break;
                }
            }
            if (!isexists) {
                msg = "[" + obj.erp_code + "]没有对应的散件包排柜!"
                strArr.push(msg);
            }
        }
        msg = "";
        var requestobj = BasemanService.RequestPostNoWait("sale_ship_warn_header", "checkis", {warn_id: $scope.data.currItem.warn_id});
        if (requestobj.pass) {
            strArr.push(requestobj.data.str_msg);
        }
        if (strArr.length > 0) {
            msg = strArr[0];
