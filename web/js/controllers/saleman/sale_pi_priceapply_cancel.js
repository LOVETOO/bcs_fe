...
function sale_pi_priceapply_cancel($scope, $location, $http, $rootScope, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService, HistoryDataService) {

    //继承基类方法
    sale_pi_priceapply_cancel = HczyCommon.extend(sale_pi_priceapply_cancel, ctrl_bill_public);
    sale_pi_priceapply_cancel.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
    ...
    $scope.search = function (e) {
        try {
            ...
            BasemanService.RequestPost("sale_pi_priceapply_header", "getcancellist", postdata).then(function (data) {
                $scope.gridSetData("view_options", data.sale_pi_priceapply_lineofsale_pi_priceapply_headers);
            })
        } catch (error) {
    ...
    $scope.cancel = function (e) {
        try {
            ...
            for (var i = 0; i < datas.length; i++) {
                ...
                var obj = BasemanService.RequestPostNoWait("sale_pi_priceapply_header", "cancel", {
...
billmanControllers
    .controller('sale_pi_priceapply_cancel', sale_pi_priceapply_cancel)
