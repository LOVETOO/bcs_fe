...
function sale_enquiry_list_headerEdit($scope, $location, $http, $rootScope, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService, HistoryDataService) {
    ...
    $scope.exportjy1 = function () {
        ...
        BasemanService.RequestPost("sale_enquiry_list_header", "exporttoexceljy1", {'enquirylist_id': $scope.data.currItem.enquirylist_id})
            .then(function (data) {
    ...
    $scope.validate=function () {
        var checktimes=0,seqstr;
        for(var i=0;i<$scope.data.currItem.sale_enquiry_list_lineofsale_enquiry_list_headers.length;i++){
            var obj=$scope.data.currItem.sale_enquiry_list_lineofsale_enquiry_list_headers[i];
            if(obj.usable==2){
                checktimes++;
            }
        }
    ...
    $scope.refresh_after = function () {
        ...
        if (window.userbean.stringofrole.indexOf("船务") != -1) {// && $scope.data.currItem.currprocname == ""
            ...
            $scope.cntcolumns[$scope.getIndexByField("cntcolumns","usable")].editable=false;
        }
        if($scope.data.currItem.currprocname=="业务确认"){
            $scope.cntcolumns[$scope.getIndexByField("cntcolumns","usable")].editable=true;
        }
