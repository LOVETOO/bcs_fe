/**
 * 引入日记账-回款 introduce_daily_book_recieved_payment
 * Created by zhl on 2019/3/14.
 */
define(
            ...
            function ($scope, $stateParams, $q, $modal) {
                ...
                $scope.gridOptions = {
                    ...
                    hcBeforeRequest: function (searchObj) {
                        searchObj.sqlwhere = '';
                        searchObj.searchflag = 13;
/*
                        //替换sqlwhere,stat = 1 ,改为 已审核、结算类型为 “现金”（settlement_type=1）或“银行”（settlement_type=2）、轧账本月
                        ...
                        requestApi.post('gl_account_period', 'getcheckoutinfo', {}).then(function (data) {
                            console.log(data.checkoutfnyearmonth, 'data.checkoutfnyearmonth');

                            $scope.data.currItem.checkoutfnyearmonth = data.checkoutfnyearmonth;
                            console.log($scope.data, 'in post');
                            return data;
                        });

                        //$scope.data.currItem.checkoutfnyearmonth   "'2019-01'"
                        var newSqlwhere = "stat = 5 and to_char(date_fund,'yyyy-MM') = " + $scope.data.currItem.checkoutfnyearmonth +
                            " and (settlement_type = 1 or settlement_type = 2)";

                        if (searchObj.sqlwhere.indexOf("stat = 1") != -1)
                            searchObj.sqlwhere = where.replace("stat = 1", newSqlwhere);
                        else
                            searchObj.sqlwhere = newSqlwhere;*/

                   /*     $scope.getCheckoutfnyearmonth = function () {
                            return requestApi.post('gl_account_period', 'getcheckoutinfo', {}).then(function (data) {
                                console.log(data.checkoutfnyearmonth, 'data.checkoutfnyearmonth');
                ...
                $scope.save = function () {
                    ...
                    loopApi.forLoop(selectRows.length, function (i) {
                        ...
                        postData.customer_id = item.customer_id;//客户
                         ...
                         *
                         * 如果“回款金额” > 0
                         * 则：借方金额 = 销售回款金额 ；贷方金额 = 0
                         *如果“回款金额” < 0（不存在等于0的情况）
                         * 则：借方金额 = 0 ；贷方金额 = 销售回款金额（取绝对值）
                         */
                        if (item.amount_debit > 0) {
                            postData.amount_debit = item.amount_debit;//借方金额
                            postData.amount_credit = 0;//贷方金额
                        } else if (item.amount_debit < 0) {
                            postData.amount_debit = 0;//借方金额
                            postData.amount_credit = Math.abs(item.amount_credit);//贷方金额
                        }
