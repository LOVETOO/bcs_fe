            ...
            function ($scope, $modal) {
                ...
                function getCurBalance() {
                    var postData = {
                        ...
                        data: {fd_fund_account_id: $scope.data.currItem.fd_fund_account_id}
                    };
                    return requestApi.post(postData)
                        .then(function (data) {
                            $scope.data.currItem.curr_amt = data.amount_blnc;
                        });
                }
                ...
                function checkData(invalidBox) {
                    if ($scope.data.currItem.amount_debit == 0 && $scope.data.currItem.amount_credit == 0) {
                        invalidBox.push('"借方金额"' + "和" + '"贷方金额"' + "不能同时为0")
                    } else if ($scope.data.currItem.amount_debit != 0 && $scope.data.currItem.amount_credit != 0) {
                        invalidBox.push('"借方金额"' + "和" + '"贷方金额"' + "必须一方为0")
                    }
                    if ($scope.data.currItem.date_fund < $scope.data.currItem.date_busine) {
                        invalidBox.push('"记账日期"' + "不能小于" + '"业务日期"');
                    }
                }

                $scope.ondatefundchange = function () {
                    $scope.data.currItem.year_month = $scope.data.currItem.date_fund.substr(0, 7);
                }
