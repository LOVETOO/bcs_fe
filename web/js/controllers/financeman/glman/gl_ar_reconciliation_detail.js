    ...
    function (module, controllerApi, base_diy_page, requestApi, loopApi, swalApi, openBizObj, numberApi, strApi) {
        ...
        var controller = [
            ...
            function ($scope, $stateParams) {
                 ...
                 * 打开应收模块详情页
                 */
                $scope.openArProp = function (params) {
                ...
                $scope.searchByCon = function (openBy) {
                    ...
                    if(strApi.isNotNull(openBy)){
                        ...
                        if('reconciliation' === openBy){
                            ...
                            $scope.gridOptions.columnDefs[$scope.getIdxByField('customer_code')].headerName
                                = "客户[" + $stateParams.object_name + "]";
                            //重设表头名
                            $scope.gridOptions.api.setColumnDefs($scope.gridOptions.columnDefs);
                        }
                    }else{
                        $scope.gridOptions.columnDefs[$scope.getIdxByField('customer_code')].headerName = "客户";
                        //重设表头名
                        $scope.gridOptions.api.setColumnDefs($scope.gridOptions.columnDefs);
                    }

                    return requestApi.post('gl_account_subject_balance', 'glarreconciliationfx', postData)
                        .then(function (data) {
