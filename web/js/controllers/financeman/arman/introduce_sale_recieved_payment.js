/**
 * 引入销售回款 introduce_sale_recieved_payment
                                    ...
                                    onCellValueChanged: function (args) {
                                        if (args)
                                            if (args.newValue === args.oldValue)
                                                return;
                                        //获取产品
                                        $scope.getCustomer(args.newValue)
...
/*
                ...
                $scope.save = function () {
                    ...
                    var searchflag = 6;//增加一条数据到【销售回款】

                    var checkRow = $scope.checkRowData(selectedNodes, selectRows);

                    if (checkRow.length > 0)
                        return swalApi.info("保存失败，【客户编码】、【品类】必须填写且是有效数据,请检查以下行：" + checkRow);
                    ...
                    loopApi.forLoop(selectRows.length, function (i) {
                        ...
                        postData.employee_id = item.employee_id;//业务员
                ...
                $scope.checkRowData = function (selectedNodes, selectRows) {
                    ...
                    for (var i = 0; i < selectRows.length; i++) {
                        ...
                        if (!selectRows[i].customer_id || selectRows[i].customer_id == 0 || !selectRows[i].crm_entid || selectRows[i].crm_entid == 0) {
