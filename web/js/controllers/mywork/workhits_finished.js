 ...
 * 工作流提醒-已完成
 */
function myWorkHitsFinished($scope,$timeout,$location, $rootScope,$state,notify,BasemanService,localeStorageService){
	//新方法
        ...
        BasemanService.RequestPost("omsworkhint", "search", postdata)
            .then(function(data) {
                ...
                for (var i = 0; i < $scope.data.currItem.workhits.length; i++) {
					...
					//var formclass = [];
					var cound = 0;
					if(dph.formclass == formclass.formclass){
						cound = 1 ;
					}
					if(cound == 0){
						formclass.push(dph.wfname);
						formclass.push(dph.formclass);
					}
						//console.log(dph.wfname);
							
						console.log(formclass);
						//console.log(workhits[i].formclass);
 ...
 angular.module('inspinia')
	.controller("myworkflow",myWorkHitsFinished)
