...
function fin_funds_headerEdit($scope, $location, $rootScope, $modal, notify, $timeout, BasemanService, $state, localeStorageService, FormValidatorService) {
    //继承基类方法
    fin_funds_headerEdit = HczyCommon.extend(fin_funds_headerEdit, ctrl_bill_public);
    fin_funds_headerEdit.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
    ...
    $scope.beforClearInfo = function () {
        ...
        if (name == "crmman.fin_funds_header_ttEdit") {//TT到款
            $scope.s_flag = 1;
            $scope.objconf.FrmInfo = {sqlBlock: "Funds_Type = 1"};
            $scope.objconf.grids = [{optionname: 'options_11', idname: 'fin_funds_sapoffin_funds_headers'}//sap凭证明细
            ]
        } else if (name == "crmman.fin_funds_header_ttrlEdit") {//客户认领
            $scope.s_flag = 2;
            $scope.objconf.FrmInfo = {sqlBlock: "confirm_stat = 2 and Funds_Type = 1"};
            $scope.objconf.grids = [{optionname: 'options_15', idname: 'fin_funds_kind_lineoffin_funds_headers'}//产品部明细
            ];
        }
    };
    ...
    /******************弹出框区域*******************/
    {
        ...
        $scope.confirm = function () {
            ...
            if ($scope.data.currItem.bank_name.indexOf("香港") > 0 && $scope.data.currItem.return_ent_type != 2) {
                ds.dialog.confirm("银行名称包含香港,但是回款组织不是香港,是否继续提交？", function () {

                }, function () {
                    $scope.newWindow.close();
                });
            }
            if ($scope.data.currItem.currency_name != $scope.data.currItem.trans_currency_name) {
                if (parseFloat($scope.data.currItem.trans_currency_amt || 0) == 0) {
                    BasemanService.notify(notify, '转换币种金额不能等于0!', "alert-warning", 1000);
                    return;
                }
                if (parseFloat($scope.data.currItem.trans_amt || 0) == 0 && parseFloat($scope.data.currItem.amt_deduct || 0) > 0) {
                    BasemanService.notify(notify, '转换币种扣费不能等于0!', "alert-warning", 1000);
                    return;
                }
                if (parseFloat($scope.data.currItem.trans_amt || 0) > 0 && parseFloat($scope.data.currItem.amt_deduct || 0) == 0) {
                    BasemanService.notify(notify, '转换币种扣费需等于0!', "alert-warning", 1000);
                    return;
                }
            }
            if ($scope.data.currItem.funds_id == undefined || $scope.data.currItem.funds_id == 0) {
                BasemanService.notify(notify, "单据没有保存，无法确认", "alert-warning", 1000);
                return;
            }
            if ($scope.data.currItem.confirm_stat == '2') {
                BasemanService.notice("单据已经确认");
                return
            }
            $scope.changeCurrency();
            $scope.changetranscurrency();
            if ($scope.data.currItem.is_into_amt == 2 && $scope.data.currItem.tt_lc_no == "") {
                BasemanService.notice('请输入对应的信用证号!');
                return
            }
            if ($scope.data.currItem.is_into_amt == 2 && $scope.data.currItem.tt_invoice_no == "") {
                BasemanService.notice('请输入对应的发票号!');
                return
            }
            if (FormValidatorService.validatorFrom($scope)) {
                ...
                var action = "confirm";
                postdata.flag = 1;
                BasemanService.RequestPost("fin_funds_header", action, postdata)
                    .then(function (data) {
                        BasemanService.notify(notify, "确认成功!", "alert-info", 1000);
        ...
        $scope.scparea = function () {
            if ($scope.s_flag == 1 && $scope.data.currItem.confirm_stat == 2) {
                return;
            }//TT到款确认
            $scope.FrmInfo = {
                classid: "scparea",
        ...
        $scope.getHxLine = function () {
            ...
            BasemanService.RequestPost("bill_invoice_header", "search", {flag: 14, sqlwhere: sql}).then(function (result) {
        ...
        $scope.scporg = function () {
            ...
            $scope.FrmInfo = {
                classid: "scporg",
                sqlBlock: $scope.orgsql + "and 1=1 and stat =2 and OrgType = 5",
                backdatas: "orgs"
            };
            BasemanService.open(CommonPopController, $scope).result.then(function (result) {
        ...
        $scope.lc_interestChange = function () {
            ...
            /*$scope.data.currItem.f_amt = $scope.data.currItem.amt || 0;//原币种首次实收金额
            $scope.data.currItem.f_amt_deduct = $scope.data.currItem.amt_deduct || 0;//原币种首次扣费
            $scope.data.currItem.f_fact_amt = $scope.data.currItem.fact_amt || 0;//原币种首次付款总额*/
            if ($scope.s_flag == 2) {
                $scope.AmtChange();
            }
            ;
        ...
        $scope.Update = function () {
            ...
            /*if($scope.data.currItem.trans_currency_id==0||$scope.data.currItem.trans_currency_id==undefined){
             postdata.trans_currency_code = $scope.data.currItem.currency_code;
             postdata.trans_currency_id = $scope.data.currItem.currency_id;
             postdata.trans_currency_name = $scope.data.currItem.currency_name;
             }*/
    ...
    /*********************保存校验区域/界面初始化*****************************/
    {
        $scope.validate = function () {
            var errorlist = [];
            //实收金额不能为负数
            if ($scope.data.currItem.amt < 0 || $scope.data.currItem.amt == undefined) {
            ...
            if ($scope.s_flag == 3) {
                ...
                if ($scope.data.currItem.is_into_amt == undefined || $scope.data.currItem.is_into_amt == "") {
            ...
            if ($scope.s_flag == 1) {
                //业务部门
                if (!($scope.data.currItem.tt_type == 3 || $scope.data.currItem.tt_type == 4 || $scope.data.currItem.tt_type == 5 || $scope.data.currItem.tt_type == 6)) {
                } else if (!($scope.data.currItem.org_id == undefined || $scope.data.currItem.org_id == "")) {
                } else {
                    errorlist.push("业务部门不能为空")
                }
                ;
                //客户
                if (!($scope.data.currItem.tt_type == 3 || $scope.data.currItem.tt_type == 4 || $scope.data.currItem.tt_type == 5 || $scope.data.currItem.tt_type == 6)) {
                } else if (!($scope.data.currItem.cust_id == undefined || $scope.data.currItem.cust_id == "")) {
                } else {
                    errorlist.push("客户不能为空")
                }
                ;
                //合同号
                if (!($scope.data.currItem.tt_type == 6)) {
                } else if (!($scope.data.currItem.hth == undefined || $scope.data.currItem.hth == "")) {
                } else {
                    errorlist.push("合同号不能为空")
                }
                ;
                //转换币种
                if($scope.data.currItem.trans_currency_id){
                    ...
                    if(($scope.data.currItem.currency_id!=$scope.data.currItem.trans_currency_id)
                        &&parseFloat($scope.data.currItem.trans_amt||0)<0){
        ...
        $scope.save_before = function () {
            ...
            if ($scope.data.currItem.currency_id != $scope.data.currItem.trans_currency_id) {
                $scope.data.currItem.trans_currency_id = $scope.data.currItem.trans_currency_id;
                $scope.data.currItem.trans_currency_code = $scope.data.currItem.trans_currency_code;
                $scope.data.currItem.trans_currency_name = $scope.data.currItem.trans_currency_name;
            }else{
            ...
            //原币种首次实收金额”自动赋值为“实收金额”
            $scope.data.currItem.f_amt = $scope.data.currItem.amt || 0;//原币种首次实收金额
            $scope.data.currItem.f_amt_deduct = $scope.data.currItem.amt_deduct || 0;//原币种首次实收金额
            $scope.data.currItem.f_fact_amt = $scope.data.currItem.fact_amt || 0;//原币种首次实收金额
        };
        ...
        $scope.clearinformation = function () {
            ...
            $scope.data.currItem.funds_type == 1;
            if (name == "crmman.fin_funds_header_ttEdit") {//TT到款
                $scope.data.currItem.tt_type = 1;
                $scope.data.currItem.confirm_stat = 1;
                $scope.data.currItem.funds_type = 1;
            } else if (name == "crmman.fin_funds_header_ttrlEdit") {//客户认领
                $scope.data.currItem.confirm_stat = 2;
                $scope.data.currItem.funds_type = 1;
            } else if (name == "crmman.fin_funds_header_lcEdit") {//信用证到款录入

            }
        };
        $scope.Org_sql = function () {
            var requestobj = BasemanService.RequestPostNoWait("base_search", "loginuserinfo", {});
            if (requestobj.pass) {
                $scope.orgsql = requestobj.data.loginuserifnos[0].like_org_ids;
            }
        };
        $scope.Org_sql();
    }
...
//加载控制器
finmanControllers
    .controller('fin_funds_headerEdit', fin_funds_headerEdit);
