    ...
    /**网格配置*/
    {
        ...
        //cntoptions
        {
            ...
            $scope.rowClicked = function (e) {
                if (e.data) {
                    if (e.data.sale_customs_lineofsale_customs_h_lines == undefined) {
                        e.data.sale_customs_lineofsale_customs_h_lines = []
                    }
                    $scope.gridSetData('cntlineoptions', e.data.sale_customs_lineofsale_customs_h_lines);
                }
                //控制明细行的可编辑
                if (e.data.new_row == '2') {
                    for (var i = 1; i < $scope.cntlinecolumns.length; i++) {
                        $scope.cntlinecolumns[i].editableBak = $scope.cntlinecolumns[i].editable;
                        $scope.cntlinecolumns[i].editable = true;
                    }
                    var field="";
                    ...
                    for (var i = 1; i < $scope.cntcolumns.length; i++) {
                        ...
                        $scope.cntcolumns[i].editableBak = $scope.cntcolumns[i].editable;
                        $scope.cntcolumns[i].editable = true;
                    }
                } else {
                    for (var i = 1; i < $scope.cntlinecolumns.length; i++) {
                        if ($scope.cntlinecolumns[i].editableBak != undefined)
                            $scope.cntlinecolumns[i].editable = $scope.cntlinecolumns[i].editableBak;
                    }

                    for (var i = 1; i < $scope.cntcolumns.length; i++) {
                        if ($scope.cntcolumns[i].editableBak != undefined)
                            $scope.cntcolumns[i].editable = $scope.cntcolumns[i].editableBak;
                    }
                }
            ...
            $scope.cellchange = function () {
                ...
                if (field == "unit_nw" || field == "unit_gw" || field == "unit_tj") {
                    ...
                    for (var j = 0; j < lines.length; j++) {
                        ...
                        nw2 += (lines[j].unit_nw || 0) * qty;
                        gw2 += (lines[j].unit_gw || 0) * qty;
                        tj2 += (lines[j].unit_tj || 0) * qty;
                        nw += (lines[j].unit_nw || 0);
                        gw += (lines[j].unit_gw || 0);
                        tj += (lines[j].unit_tj || 0);
                    }
                    h_detail.unit_nw = nw2;
                    h_detail.unit_gw = gw2;
                    h_detail.unit_tj = tj2;
                    nw = nw;
                    gw = gw;
                    tj = tj;

                    // h_detail.customs_amt=(h_detail.qty||0)*h_detail.qty;

                    key = ["unit_nw", "unit_gw", "unit_tj", "nw", "gw", "tj"]
                    $scope["cntoptions"].api.refreshCells(nodes, key);
                    return;
                }
                if (field == "customs_price") {
                    var price = Number(h_detail.customs_price || 0);
                    var index = $scope[options].api.getFocusedCell().rowIndex;
                    var linedata = $scope.gridGetRow([options]);
                    var nodes = $scope.gridGetNodes(options);
                    if (lines.length == 1) {
                        ...
                        for (var j = 0; j < lines.length; j++) {
                            if (j != index) {
                                ...
                                $scope[options].api.refreshCells([nodes[j]], ["customs_price", "customs_amt"])
                                $scope[options].api.refreshCells([nodes[index]], ["customs_amt"])
                                break;
                            }
                        }
                    }
                ...
                if (h_detail.new_row == "2") {
                    h_detail[field] = val;
                    var hnodes = $scope.gridGetNodes("cntoptions");
                    $scope["cntoptions"].api.refreshCells(hnodes, ["customs_amt"])
                    return;
                }
                $scope.partSummary();
            }
