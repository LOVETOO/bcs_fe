...
function fin_funds_m_header_lcEdit($scope, $location, $rootScope, $modal, notify, $timeout, BasemanService, $state, localeStorageService, FormValidatorService) {
    //继承基类方法
    fin_funds_m_header_lcEdit = HczyCommon.extend(fin_funds_m_header_lcEdit, ctrl_bill_public);
    fin_funds_m_header_lcEdit.__super__.constructor.apply(this, [$rootScope, $scope, $location, $modal, $timeout, BasemanService, $state, localeStorageService, FormValidatorService]);
    //设置当前单据对象基本信息
    $scope.objconf = {
    ...
    $scope.beforClearInfo = function () {
        ...
        if (name == "crmman.fin_funds_m_header_lcEdit") {//信用证到款录入变更
            $scope.s_flag = 5;
    ...
    /****************网格事件**************/
    {
        ...
        $scope.ChangeAmt = function () {
            ...
            itemtemp.fact_amt = parseFloat(val || 0) + parseFloat(itemtemp.amt_deduct||0);
            $scope.options_11.api.refreshCells(nodes, ["fact_amt"]);

        };
        //扣费
        $scope.ChangeAmt_deduct = function () {
            var _this = $(this);
            var val = _this.val();
            var index = $scope.options_11.api.getFocusedCell().rowIndex;
            var nodes = $scope.options_11.api.getModel().rootNode.childrenAfterSort;
            var itemtemp = nodes[index].data;
            var pi_item_h_lines = itemtemp.sale_pi_item_lineofsale_pi_item_h_lines;
            itemtemp.fact_amt = parseFloat(val || 0) + parseFloat(itemtemp.amt||0);
            $scope.options_11.api.refreshCells(nodes, ["fact_amt"]);
        ...
        $scope.fin_funds_header = function () {
            $scope.FrmInfo = {
                ...
                searchlist: ["funds_no", "other_no", "fact_amt",
                    "allocated_amt", "confirm_date", "confirm_man", "org_name", "cust_name", "confirm_date", "confirm_man", "note"],
            };
            var name = $rootScope.$state.$current.self.name;
            ...
            BasemanService.open(CommonPopController, $scope, '', 'lg').result.then(function (result) {
                ...
                BasemanService.RequestPost("fin_funds_header", "select", {funds_id: result.funds_id})
                    .then(function (data) {
                        ...
                        var items=data.fin_funds_lineoffin_funds_headers;
                        if (items.length > 0) {
                            var lcs = $scope.gridGetData("options_12");
                            var isExists = false;
                            for (var i = 0; i < items.length; i++) {
                                var isExists = HczyCommon.isExist(lcs, items[i], ["invoice_id", "invoice_no"], ["invoice_id", "invoice_no"]).exist;
                                if (isExists) {
                                    continue;
                                }
                                items[i].old_allot_amt = Number(items[i].allot_amt)||0;
    ...
    /**************网格定义区域***************/
    {
        ...
        $scope.columns_12 = [
            ...
            , {
                ...
                headerName: "此次分配金额", field: "allot_amt", editable: true, filter: 'set', width: 130,
                cellEditor: "浮点框",
                enableRowGroup: true,
                enablePivot: true,
                enableValue: true,
        ...
        $scope.columns_11 = [
            {
                ...
                headerName: "实收金额", field: "amt", editable: true, filter: 'set', width: 100,
                cellEditor: "浮点框",
                cellchange: $scope.ChangeAmt,
                enableRowGroup: true,
                enablePivot: true,
                enableValue: true,
                floatCell: true
            }, {
                headerName: "扣费", field: "amt_deduct", editable: true, filter: 'set', width: 150,
                cellEditor: "浮点框",
                cellchange: $scope.ChangeAmt_deduct,
                enableRowGroup: true,
                enablePivot: true,
                enableValue: true,
                floatCell: true
    ...
    /************保存校验/界面初始化/提交*************/
    {
        ...
        $scope.wfstart = function (e) {
            ...
            BasemanService.RequestPost("fin_funds_m_header", "lcmcheck", {funds_m_id: $scope.data.currItem.funds_m_id}).then(function (data) {
...
//加载控制器
basemanControllers
    .controller('fin_funds_m_header_lcEdit', fin_funds_m_header_lcEdit);
