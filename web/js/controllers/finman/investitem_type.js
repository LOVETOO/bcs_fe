    ...
    function (module, controllerApi, base_diy_page, swalApi, requestApi) {
        ...
        var controller = [
            ...
            function ($scope) {
                ...
                // 查询
                $scope.gridOptions.hcClassId = 'fin_investitem_type';
                ...
                $scope.delete = function () {
                    // 获取选中的行的数据
                    var focuseData = $scope.gridOptions.hcApi.getFocusedData();
                    if (!focuseData || !focuseData.invitem_type_id) {
                        return swalApi.info("请选择要删除的行!");
                    };
                    return swalApi.confirmThenSuccess({
                        title: "确定要删除名称为" + focuseData.invitem_type_name + "的项目吗?",
                        okFun: function () {
                            //函数区域
                            requestApi.post("fin_investitem_type", "delete", {
                                invitem_type_id: focuseData.invitem_type_id
                            }).then(function (data) {
                                $scope.refresh();
                                //清除选中
                ...
                //保存
                $scope.save = function () {
                    if (!$scope.data.currItem.invitem_type_name) {
                        return swalApi.error("投资项目名称不能为空!");
                    }
                    if (!$scope.data.currItem.invitem_type_id) {
                        //新增
                        requestApi.post("fin_investitem_type", "insert", $scope.data.currItem).then(function (data) {
                            $scope.refresh();
                            //清除选中
                            $scope.gridOptions.api.focusedCellController.clearFocusedCell();
                        ...
                        //更新
                        requestApi.post("fin_investitem_type", "update", $scope.data.currItem).then(function (data) {
                            $scope.refresh();
                            //清除选中
                            $scope.gridOptions.api.focusedCellController.clearFocusedCell();
                            return swalApi.success("保存成功!");
