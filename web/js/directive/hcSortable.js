 ...
 * 由 AngularUI 实现
 * @version v0.19.0
 * @link http://angular-ui.github.io/ui-sortable/
 * @link https://github.com/angular-ui/ui-sortable
 * @since 2019-03-14
 */
(function (defineFn) {
	define(['module', 'directiveApi', 'angular', 'app', 'jquery-ui'], defineFn);
})(function (module,   directiveApi,   angular,   app) {

	app.value('hcSortableConfig', {
		...
		//http://www.runoob.com/jqueryui/api-sortable.html
		//修改插件的部分默认值，使他更适合我们系统使用
		...
		//鼠标按下后排序开始前必须移动的距离，以像素计。
		//如果指定了该选项，排序只有在鼠标拖拽超出指定距离时才会开始。
		//原始默认值：1
		distance: 5,
	...
	hcSortableDirective.$inject = ['hcSortableConfig', '$timeout', '$log'];
	function hcSortableDirective(   hcSortableConfig,   $timeout,   $log) {
		return {
			require: '?ngModel',
			scope: {
				ngModel: '=',
				...
				create: '&hcSortableCreate',
				// helper:'&hcSortableHelper',
				start: '&hcSortableStart',
				activate: '&hcSortableActivate',
				// sort:'&hcSortableSort',
				// change:'&hcSortableChange',
				// over:'&hcSortableOver',
				// out:'&hcSortableOut',
				beforeStop: '&hcSortableBeforeStop',
				update: '&hcSortableUpdate',
				remove: '&hcSortableRemove',
				receive: '&hcSortableReceive',
				deactivate: '&hcSortableDeactivate',
				stop: '&hcSortableStop'
			},
			link: function hcSortableLink(scope, element, attrs, ngModel) {
				var savedNodes;
				var helper;
				...
				function patchSortableOption(key, value) {
					...
					if (!value && (key === 'items' || key === 'hc-model-items')) {
						value = hcSortableConfig.items;
					}
				...
				function getPlaceholderExcludesludes(element, placeholder) {
					...
					var notCssSelector = opts['hc-model-items'].replace(/[^,]*>/g, '');
				...
				function getSortingHelper(element, ui /*, savedNodes*/) {
					...
					if (
						...
						// The .hc-sortable-helper element (that's the default class name)
						result = helper;
					}
					return result;
				}
				...
				var directiveOpts = {
					'hc-floating': undefined,
					'hc-model-items': hcSortableConfig.items,
					'hc-preserve-size': undefined
				};
				...
				function wireUp() {
					...
					scope.$watchCollection('ngModel', function () {
						...
						$timeout(
							function () {
								// ensure that the jquery-hc-sortable widget instance
					...
					callbacks.update = function (e, ui) {
						...
						if (!ui.item.sortable.received) {
							...
							var droptarget = ui.item
								.parent()
								.closest(
									'[hc-sortable], [data-hc-sortable], [x-hc-sortable]'
								);
						...
						if (ui.item.sortable.received && !ui.item.sortable.isCanceled()) {
							...
							scope.$emit('hc-sortable:moved', ui);
						}
					};
				...
				function initIfEnabled() {
					if (scope.hcSortable && scope.hcSortable.disabled) {
						return false;
					}
