...
/*                $('#div_doc_menu').menu('show',{
    ...
    if (shtTable.fixlen == 2) {
		var tableid = $(event.target).attr("cpctableid");
		var rowspanobj = $($("[cpctableid="+tableid+"]").parent().prev().children('[rowspan]'));
		var rowspan = $($("[cpctableid="+tableid+"]").parent().prev().children('[rowspan]')).attr("rowspan");
                ...
                onClick: function (item) {
                    ...
                    switch (item.id) {
                        case 'insert-one-row':
                            ...
                            shtTable.rowcount+= 1;
							if(rowspanobj && rowspan > 2){
								rowspanobj.attr("rowspan",parseInt(rowspan)+1);
							}
                            break;
                        case 'insert-two-row':
                            console.log('插入行(前面)');
                            ...
                            shtTable.rowcount+= 2;
							if(rowspanobj && rowspan > 2){
								rowspanobj.attr("rowspan",parseInt(rowspan)+2);
							}
                            break;
                            ...
                            $.messager.prompt('插入自定义行','输入自定义行数',function (r) {
                                if (r) {
                                    if ((/^(\+|-)?\d+$/.test(r)) && r > 0) {
                                        ...
                                        shtTable.rowcount+= r;
										if(rowspanobj && rowspan > 2){
											rowspanobj.attr("rowspan",parseInt(rowspan)+parseInt(r));
										}
                                    } else {
...
CellEditor.prototype.getEditCtrl = function () {
    ...
    if (3 == this.shtCol.coltype) {
        ...
        editCtrl = $(str).val(value);
		 ...
		 this.shtCol.getColStyleValue(this.rowNo);
        // 1.下拉框不能输入
        str = "<select id='c_";
        str += this.colid;
        str += "'";
...
CPCShtGrid.prototype.onSelectUser = function (isOrg) {
	...
	if(parseInt(that.activeEditor.shtCol.colstyle) == 7||parseInt(that.activeEditor.shtCol.colstyle) == 8){
		 isNotMulti = false
	}else{
		 isNotMulti = true;
	}
    ...
    $('#select_user').dialog({
        ...
        modal: true,
        queryParams: {isNotMulti: isNotMulti},
        buttons:[{
            text:'确定',
            iconCls: 'icon-ok',
            handler:function(){
				...
				//单选
				if(!isNotMulti){
					value = that.activeEditor.newValue
					text = that.activeEditor.oldText
					for(var i=0;i<selectedUser.length;i++){
					
						if(value==''||value==undefined){
							value = selectedUser[i].userid;
							text = selectedUser[i].username;
						}else{
							if(value.indexOf(selectedUser[i].userid)<0){
								value += ';'+selectedUser[i].userid;
							    text += ';'+selectedUser[i].username;
							}
                        ...
                        success: function(data){
                            ...
                            if (response.success) {
                                ...
                                for (var i=0;i<data.length;i++) {
                                    ...
                                    docNames[i] = data[i].docname + "=download/downloadfile.do?iswb=true&filecode=" + data[i].downloadcode+"=download"+data[i].oldsize;
                                }
                                value = docIds.join(';');
                                text = docNames.join(';');
                                if (value) {
