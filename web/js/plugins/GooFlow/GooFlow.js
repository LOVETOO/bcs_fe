...
//初始化区域图的对象
    ...
    height = height - (property.haveHead ? 5 : 8);
    this.$bgDiv.append("<div class='GooFlow_work' style='" + (property.haveHead ? "top:" + headHeight + "px;" : "") + (property.haveTool ? "left:34px" : "") + "'></div>");
...
GooFlow.prototype = {
    ...
    addNode: function (id, json) {
        ...
        var users = '';
        // if (json.procusers && json.procusers.length > 0) {
        //     for (var i = 0; i < json.procusers.length; i++) {
        //         users += "<i class='ico_" + json.type + "'></i> " + json.procusers[i].username + '<br/>';
        //     }
        // }
        if (json.procusers && json.procusers.length > 0) {
            for (var i = 0; i < json.procusers.length; i++) {
                if (json.procuserofwfprocs[i].issigned==2) {
                    users += "<div class='approve'><i class='fa fa-check'></i>" + json.procusers[i].username + "</div>";
                } else{
                    users += "<div><i class='ico_" + json.type + "'></i> " + json.procusers[i].username + "</div>";
                }
            }
        }

        var time = '';
        if (json.aendtime && json.aendtime.length > 0) {
                time = "<div class='time'>" + json.aendtime + "</div>";
        }
        ...
        var reg=json.name.replace(/[^\u4e00-\u9fa5]/ig,"");

        //增加头部标题列
        var header = "<div class='item_header' title='" + json.name + "'><span>" + json.name + "</span></div>";
        ...
        if (json.type.indexOf(" round") < 0) {
            ...
            this.$nodeDom[id] = $("<div class='GooFlow_item "
                + cstat + mark + "' id='" + id + "' style='top:"
                + json.top * this.$scale + "px;left:"
                + json.left * this.$scale + "px'>"
                + "<table cellspacing='1' style='width:"
                + (json.width * this.$scale - 10) + "px;height:" + (json.height * this.$scale - 10)
                + "px;'><tr><td class='ico'></td><td>"
                + header +
                "<div class='users'>" + users +"</div>" +
                "<div class='state'>状态:" + state + "</div>"
                + time +
            ...
            json.width = 38;
            json.height = 38;
            this.$nodeDom[id] = $("<div class='GooFlow_item item_round" + mark + "' id='" + id
                + "' style='top:" + json.top * this.$scale + "px;left:" + json.left * this.$scale
                + "px'><table cellspacing='0' style='width:" + (json.width * this.$scale - 2)
                + "px;height:" + (json.height * this.$scale - 2)
                + "px;'><tr><td class='ico'><i class='ico_" + json.type
                + "'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>"
                + reg + "</div></div>");
        }
    ...
    initWorkForNode: function () {
        ...
        //绑定用鼠标移动事件
        this.$workArea.delegate(".ico,.item_header", "mousedown", {inthis: this}, function (e) {
            if (!e) e = window.event;
            if (e.button == 2) return false;
            var This = e.data.inthis;
    ...
    //清空工作区及已载入的数据
    //trigger 是否触发删除事件，默认否
    clearData: function (trigger) {
        if (trigger === undefined) trigger = false;

        for (var key in this.$nodeData) {
            this.delNode(key, trigger);
        }
        for (var key in this.$lineData) {
            this.delLine(key, trigger);
        }
        for (var key in this.$areaData) {
            this.delArea(key, trigger);
        }
        this.$deletedItem = {};
    },
    //销毁自己
...
///////////以下为有关画线的方法
    ...
    calcStartEnd: function (n1, n2, scale) {
        ...
        var X_1, Y_1, X_2, Y_2;
        //X判断：
        var x11 = n1.left * scale, x12 = n1.left * scale + n1.width * scale, x21 = n2.left * scale,
        ...
        //Y判断：
        var y11 = n1.top * scale, y12 = n1.top * scale + n1.height * scale, y21 = n2.top * scale,
    ...
    reinitSize: function (width, height) {
        ...
        //qch
        this.$bgDiv.css({height: h + "px", width: w + 2 + "px"});
