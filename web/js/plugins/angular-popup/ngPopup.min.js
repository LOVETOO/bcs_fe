...
;(function(){
    ...
    function NgPopupBuilder($q, $http,$log){
        ...
        function __updateBindingValue(newValue, oldValue, linkParams){
            ...
            $element.style.zIndex = 1100 + newValue.index;
            ...
            return {
                ...
                maximize: function(isMax){
                    if(!isMax) {
                        ...
                        var p_left = 0;
                        if($($element).closest("div#page-wrapper")){
                        	p_left = $($element).closest("div#page-wrapper")[0].offsetLeft;
                        }
                        
                        options.position.top = $element.offsetTop;
                        options.position.left = $element.offsetLeft - p_left;
                        options.width = $element.offsetWidth;
                        options.height = $element.offsetHeight;
...
;(function(){
    ...
    function NgPopup($document, $timeout, $q,$compile, ngPopupBuilder){
        return{
            restrict: "EA",
            scope:{
                option: "=?"
...
//                            $document.find('body').addClass('unselectable');
                            ...
                            var _this_pup = $(target).closest("div.ngPopup");
                            var _curr_index = _this_pup.css("z-index");
                            
                            var _this_pup_size = _this_pup.closest("ul.popuplist").children("li").size();
                            
                            _this_pup.closest("ul.popuplist").children("li").children("div.ngPopup").css({'z-index':1100});
                            if(_this_pup_size > 1)_this_pup.css({'z-index':1101});
                        }
